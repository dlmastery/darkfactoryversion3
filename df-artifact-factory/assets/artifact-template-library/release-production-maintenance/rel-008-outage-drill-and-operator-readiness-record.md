...
# REL-008 Outage Drill and Operator Readiness Record Template

Status: template plus fictional worked example.

Template family: `release-production-maintenance`.

Use this template when a DFMS factory run needs `Outage Drill and Operator Readiness Record` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: SRE human-owned operations.
...
| --- | --- |
| Artifact ID | REL-008 |
| Artifact name | Outage Drill and Operator Readiness Record |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Outage Drill and Operator Readiness Record` evidence;
...
- Related artifacts: Scenario, runbook replay, diagnosis, mitigation, operator signoff.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: SRE Lead, Human Owner, Support Lead.

## Required Links

Catalog required links: Scenario, runbook replay, diagnosis, mitigation, operator signoff.
...
**Example:** `Outage Drill and Operator Readiness Record` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `outage scenario` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Operational Procedure

**Fill:** Provide step-by-step deployment, rollback, monitoring, support, and incident response instructions.

**Example:** In CareQueue Pro, record how `operator actions` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Human Readiness

**Fill:** Define training, operator signoff, support readiness, customer communication, and escalation.

**Example:** In CareQueue Pro, record how `timing` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Known Issues and Risk

**Fill:** List residual risks, no-go criteria, accepted limitations, and revalidation triggers.

**Example:** In CareQueue Pro, record how `signoff` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Post-Release Learning

**Fill:** Capture feedback, incidents, metrics, maintenance actions, and project-book updates.

**Example:** In CareQueue Pro, record how `learning gaps` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Outage Drill and Operator Readiness Record` focus: outage scenario | TST-ACPT-019, REV-REL-008-001 | Example only |
...
1. Artifact captures outage scenario with owner, evidence, and trace links.
2. Artifact captures operator actions with owner, evidence, and trace links.
3. Artifact captures timing with owner, evidence, and trace links.
4. Artifact captures signoff with owner, evidence, and trace links.
5. Artifact captures learning gaps with owner, evidence, and trace links.
...
1. REL-008-C1 SRE Drill Critic: validates outage scenario, diagnosis, mitigation, rollback, and timing.
2. REL-008-C2 Human Operator Critic: validates operator signoff, learning evidence, confidence, and gaps.
3. REL-008-C3 Support Communication Critic: validates customer impact, escalation, comms, and post-drill learning.
...
| Example review panel | `reviews/rel-008-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: outage scenario, operator actions, timing, signoff, learning gaps.
- Example decision: accept this artifact only after the SRE Lead, Human Owner, Support Lead panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Outage Drill and Operator Readiness Record` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `outage scenario`, `operator actions`, and `timing` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `outage scenario`.
