...
# REL-003 Deployment Guide Template

Status: template plus fictional worked example.

Template family: `release-production-maintenance`.

Use this template when a DFMS factory run needs `Deployment Guide` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: SRE/operations.
...
| --- | --- |
| Artifact ID | REL-003 |
| Artifact name | Deployment Guide |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Deployment Guide` evidence;
...
- Related artifacts: Environments, config, rollback.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: SRE Lead, Security Engineer, Release Manager.

## Required Links

Catalog required links: Environments, config, rollback.
...
**Example:** `Deployment Guide` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `deployment steps` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Operational Procedure

**Fill:** Provide step-by-step deployment, rollback, monitoring, support, and incident response instructions.

**Example:** In CareQueue Pro, record how `prechecks` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Human Readiness

**Fill:** Define training, operator signoff, support readiness, customer communication, and escalation.

**Example:** In CareQueue Pro, record how `postchecks` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Deployment Guide` focus: deployment steps | TST-ACPT-019, REV-REL-003-001 | Example only |
...
1. Artifact captures deployment steps with owner, evidence, and trace links.
2. Artifact captures prechecks with owner, evidence, and trace links.
3. Artifact captures postchecks with owner, evidence, and trace links.
...
1. REL-003-C1 Deployment Procedure Critic: validates steps, prerequisites, environments, automation, and verification.
2. REL-003-C2 Security Deployment Critic: validates secrets, permissions, network, config, and hardening.
3. REL-003-C3 Release Control Critic: validates rollback, go/no-go, ownership, and change-window readiness.
...
| Example review panel | `reviews/rel-003-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: deployment steps, prechecks, postchecks, rollback, ownership.
- Example decision: accept this artifact only after the SRE Lead, Security Engineer, Release Manager panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Deployment Guide` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `deployment steps`, `prechecks`, and `postchecks` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `deployment steps`.
