...
# REL-001 Release Plan Template

Status: template plus fictional worked example.

Template family: `release-production-maintenance`.

Use this template when a DFMS factory run needs `Release Plan` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: RUP transition, DevOps.
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Release Plan` evidence;
...
- Related artifacts: Change set, risks, deployment steps.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Release Manager, SRE Lead, Product Owner.

## Required Links

Catalog required links: Change set, risks, deployment steps.
...
**Example:** `Release Plan` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `release scope` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Operational Procedure

**Fill:** Provide step-by-step deployment, rollback, monitoring, support, and incident response instructions.

**Example:** In CareQueue Pro, record how `go/no-go` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Human Readiness

**Fill:** Define training, operator signoff, support readiness, customer communication, and escalation.

**Example:** In CareQueue Pro, record how `deployment sequence` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
**Example:** In CareQueue Pro, record how `communication` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Release Plan` focus: release scope | TST-ACPT-019, REV-REL-001-001 | Example only |
...
1. Artifact captures release scope with owner, evidence, and trace links.
2. Artifact captures go/no-go with owner, evidence, and trace links.
3. Artifact captures deployment sequence with owner, evidence, and trace links.
4. Artifact captures rollback with owner, evidence, and trace links.
5. Artifact captures communication with owner, evidence, and trace links.
...
1. REL-001-C1 Release Strategy Critic: validates release scope, version, sequencing, dependencies, and go/no-go criteria.
2. REL-001-C2 SRE Readiness Critic: validates deployment, rollback, monitoring, runbook, and incident readiness.
3. REL-001-C3 Product Owner Impact Critic: validates user impact, comms, training, known issues, and acceptance.
...
| Example review panel | `reviews/rel-001-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: release scope, go/no-go, deployment sequence, rollback, communication.
- Example decision: accept this artifact only after the Release Manager, SRE Lead, Product Owner panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Release Plan` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `release scope`, `go/no-go`, and `deployment sequence` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `release scope`.
