...
# REL-006 Maintenance Guide Template

Status: template plus fictional worked example.

Template family: `release-production-maintenance`.

Use this template when a DFMS factory run needs `Maintenance Guide` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: ISO maintenance.
...
| --- | --- |
| Artifact ID | REL-006 |
| Artifact name | Maintenance Guide |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Maintenance Guide` evidence;
...
- Related artifacts: Code map, common changes, tests.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Brownfield Maintainer, Implementation Lead, SRE Lead.

## Required Links

Catalog required links: Code map, common changes, tests.
...
**Example:** `Maintenance Guide` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `code map` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Operational Procedure

**Fill:** Provide step-by-step deployment, rollback, monitoring, support, and incident response instructions.

**Example:** In CareQueue Pro, record how `common changes` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Human Readiness

**Fill:** Define training, operator signoff, support readiness, customer communication, and escalation.

**Example:** In CareQueue Pro, record how `regression commands` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Known Issues and Risk

**Fill:** List residual risks, no-go criteria, accepted limitations, and revalidation triggers.

**Example:** In CareQueue Pro, record how `operational tasks` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Post-Release Learning

**Fill:** Capture feedback, incidents, metrics, maintenance actions, and project-book updates.

**Example:** In CareQueue Pro, record how `maintenance risks` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Maintenance Guide` focus: code map | TST-ACPT-019, REV-REL-006-001 | Example only |
...
1. Artifact captures code map with owner, evidence, and trace links.
2. Artifact captures common changes with owner, evidence, and trace links.
3. Artifact captures regression commands with owner, evidence, and trace links.
4. Artifact captures operational tasks with owner, evidence, and trace links.
5. Artifact captures maintenance risks with owner, evidence, and trace links.
...
1. REL-006-C1 Brownfield Maintenance Critic: validates code map, common changes, current behavior, and regression path.
2. REL-006-C2 Implementation Critic: validates developer workflow, dependencies, extension points, and safe-change guidance.
3. REL-006-C3 SRE Maintenance Critic: validates operational tasks, monitoring, routine checks, and escalation.
...
| Example review panel | `reviews/rel-006-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: code map, common changes, regression commands, operational tasks, maintenance risks.
- Example decision: accept this artifact only after the Brownfield Maintainer, Implementation Lead, SRE Lead panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Maintenance Guide` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `code map`, `common changes`, and `regression commands` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `code map`.
