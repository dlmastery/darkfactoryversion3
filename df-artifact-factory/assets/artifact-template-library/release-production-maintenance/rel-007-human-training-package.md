...
# REL-007 Human Training Package Template

Status: template plus fictional worked example.

Template family: `release-production-maintenance`.

Use this template when a DFMS factory run needs `Human Training Package` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: Outsourcing handoff.
...
| --- | --- |
| Artifact ID | REL-007 |
| Artifact name | Human Training Package |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Human Training Package` evidence;
...
- Related artifacts: Project book, runbooks, walkthroughs.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Human Owner, Support Lead, Product Expert.

## Required Links

Catalog required links: Project book, runbooks, walkthroughs.
...
**Example:** `Human Training Package` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `training goals` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Operational Procedure

**Fill:** Provide step-by-step deployment, rollback, monitoring, support, and incident response instructions.

**Example:** In CareQueue Pro, record how `walkthrough` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Human Readiness

**Fill:** Define training, operator signoff, support readiness, customer communication, and escalation.

**Example:** In CareQueue Pro, record how `exercises` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Known Issues and Risk

**Fill:** List residual risks, no-go criteria, accepted limitations, and revalidation triggers.

**Example:** In CareQueue Pro, record how `comprehension checks` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Human Training Package` focus: training goals | TST-ACPT-019, REV-REL-007-001 | Example only |
...
1. Artifact captures training goals with owner, evidence, and trace links.
2. Artifact captures walkthrough with owner, evidence, and trace links.
3. Artifact captures exercises with owner, evidence, and trace links.
4. Artifact captures comprehension checks with owner, evidence, and trace links.
5. Artifact captures signoff with owner, evidence, and trace links.
...
1. REL-007-C1 Human Learning Critic: validates walkthroughs, exercises, comprehension checks, and role-specific learning.
2. REL-007-C2 Support Training Critic: validates support scenarios, FAQs, escalation, and customer explanation.
3. REL-007-C3 Product Workflow Critic: validates training against actual user workflows, decisions, and success criteria.
...
| Example review panel | `reviews/rel-007-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: training goals, walkthrough, exercises, comprehension checks, signoff.
- Example decision: accept this artifact only after the Human Owner, Support Lead, Product Expert panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Human Training Package` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `training goals`, `walkthrough`, and `exercises` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `training goals`.
