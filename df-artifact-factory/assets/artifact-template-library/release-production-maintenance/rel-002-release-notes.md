...
# REL-002 Release Notes Template

Status: template plus fictional worked example.

Template family: `release-production-maintenance`.

Use this template when a DFMS factory run needs `Release Notes` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: Handoff practice.
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Release Notes` evidence;
...
- Related artifacts: Features, fixes, risks, known issues.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Product Expert, Release Manager, Support Lead.

## Required Links

Catalog required links: Features, fixes, risks, known issues.
...
**Example:** `Release Notes` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `user-facing changes` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Operational Procedure

**Fill:** Provide step-by-step deployment, rollback, monitoring, support, and incident response instructions.

**Example:** In CareQueue Pro, record how `known issues` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Human Readiness

**Fill:** Define training, operator signoff, support readiness, customer communication, and escalation.

**Example:** In CareQueue Pro, record how `upgrade notes` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Known Issues and Risk

**Fill:** List residual risks, no-go criteria, accepted limitations, and revalidation triggers.

**Example:** In CareQueue Pro, record how `support impact` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Post-Release Learning

**Fill:** Capture feedback, incidents, metrics, maintenance actions, and project-book updates.

**Example:** In CareQueue Pro, record how `risk notes` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Release Notes` focus: user-facing changes | TST-ACPT-019, REV-REL-002-001 | Example only |
...
1. Artifact captures user-facing changes with owner, evidence, and trace links.
2. Artifact captures known issues with owner, evidence, and trace links.
3. Artifact captures upgrade notes with owner, evidence, and trace links.
4. Artifact captures support impact with owner, evidence, and trace links.
5. Artifact captures risk notes with owner, evidence, and trace links.
...
1. REL-002-C1 Product Communication Critic: validates feature/fix clarity, user value, limitations, and known issues.
2. REL-002-C2 Release Governance Critic: validates version, scope, risks, approvals, and support handoff.
3. REL-002-C3 Support Readiness Critic: validates troubleshooting hints, FAQs, escalation, and customer language.
...
| Example review panel | `reviews/rel-002-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: user-facing changes, known issues, upgrade notes, support impact, risk notes.
- Example decision: accept this artifact only after the Product Expert, Release Manager, Support Lead panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Release Notes` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `user-facing changes`, `known issues`, and `upgrade notes` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `user-facing changes`.
