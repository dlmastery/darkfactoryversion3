...
# VNV-006 Performance/Reliability Report Template

Status: template plus fictional worked example.

Template family: `verification-validation`.

Use this template when a DFMS factory run needs `Performance/Reliability Report` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: SRE/NFR.
...
| --- | --- |
| Artifact ID | VNV-006 |
| Artifact name | Performance/Reliability Report |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Performance/Reliability Report` evidence;
...
- Related artifacts: NFRs, SLIs, capacity assumptions.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: SRE Lead, Test Lead, System Architect.

## Required Links

Catalog required links: NFRs, SLIs, capacity assumptions.
...
**Example:** `Performance/Reliability Report` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `load model` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Test Design

**Fill:** Specify levels, procedures, data, fixtures, negative cases, edge cases, and accessibility/security/NFR coverage.

**Example:** In CareQueue Pro, record how `SLI results` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Execution Evidence

**Fill:** Record commands, logs, screenshots, reports, result interpretation, failures, and rerun instructions.

**Example:** In CareQueue Pro, record how `capacity` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Coverage and Gaps

**Fill:** Map results to requirements, risks, code, design, and residual gaps.

**Example:** In CareQueue Pro, record how `bottlenecks` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Exit Decision

**Fill:** State pass/fail/conditional verdict, fixes, owner, revalidation, and certificate linkage.

**Example:** In CareQueue Pro, record how `reliability decision` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Performance/Reliability Report` focus: load model | TST-ACPT-019, REV-VNV-006-001 | Example only |
...
1. Artifact captures load model with owner, evidence, and trace links.
2. Artifact captures SLI results with owner, evidence, and trace links.
3. Artifact captures capacity with owner, evidence, and trace links.
4. Artifact captures bottlenecks with owner, evidence, and trace links.
5. Artifact captures reliability decision with owner, evidence, and trace links.
...
1. VNV-006-C1 SRE Performance Critic: validates load model, SLIs/SLOs, saturation, capacity, and reliability findings.
2. VNV-006-C2 Test Method Critic: validates environment, repeatability, data, tooling, and confidence.
3. VNV-006-C3 Architecture Critic: validates bottlenecks, scaling path, failure modes, and design implications.
...
| Example review panel | `reviews/vnv-006-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: load model, SLI results, capacity, bottlenecks, reliability decision.
- Example decision: accept this artifact only after the SRE Lead, Test Lead, System Architect panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Performance/Reliability Report` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `load model`, `SLI results`, and `capacity` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `load model`.
