...
# VNV-001 Master Test Strategy Template

Status: template plus fictional worked example.

Template family: `verification-validation`.

Use this template when a DFMS factory run needs `Master Test Strategy` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: ISO verification/validation, TDD/BDD.
...
| --- | --- |
| Artifact ID | VNV-001 |
| Artifact name | Master Test Strategy |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Master Test Strategy` evidence;
...
- Related artifacts: Requirements, risks, coverage targets.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Test Lead, Product Expert, Security Engineer.

## Required Links

Catalog required links: Requirements, risks, coverage targets.
...
**Example:** `Master Test Strategy` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `test levels` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Test Design

**Fill:** Specify levels, procedures, data, fixtures, negative cases, edge cases, and accessibility/security/NFR coverage.

**Example:** In CareQueue Pro, record how `risk coverage` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Execution Evidence

**Fill:** Record commands, logs, screenshots, reports, result interpretation, failures, and rerun instructions.

**Example:** In CareQueue Pro, record how `automation strategy` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Coverage and Gaps

**Fill:** Map results to requirements, risks, code, design, and residual gaps.

**Example:** In CareQueue Pro, record how `manual checks` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Exit Decision

**Fill:** State pass/fail/conditional verdict, fixes, owner, revalidation, and certificate linkage.

**Example:** In CareQueue Pro, record how `exit criteria` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Master Test Strategy` focus: test levels | TST-ACPT-019, REV-VNV-001-001 | Example only |
...
1. Artifact captures test levels with owner, evidence, and trace links.
2. Artifact captures risk coverage with owner, evidence, and trace links.
3. Artifact captures automation strategy with owner, evidence, and trace links.
4. Artifact captures manual checks with owner, evidence, and trace links.
5. Artifact captures exit criteria with owner, evidence, and trace links.
...
1. VNV-001-C1 Verification Architecture Critic: validates test levels, risk weighting, coverage model, and exit gates.
2. VNV-001-C2 Product Acceptance Critic: validates user outcomes, acceptance scenarios, and customer validation path.
3. VNV-001-C3 Security and NFR Test Critic: validates non-functional, security, performance, accessibility, and reliability tests.
...
| Example review panel | `reviews/vnv-001-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: test levels, risk coverage, automation strategy, manual checks, exit criteria.
- Example decision: accept this artifact only after the Test Lead, Product Expert, Security Engineer panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Master Test Strategy` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `test levels`, `risk coverage`, and `automation strategy` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `test levels`.
