...
# VNV-003 Automated Test Evidence Template

Status: template plus fictional worked example.

Template family: `verification-validation`.

Use this template when a DFMS factory run needs `Automated Test Evidence` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: TDD/CI.
...
| --- | --- |
| Artifact ID | VNV-003 |
| Artifact name | Automated Test Evidence |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Automated Test Evidence` evidence;
...
- Related artifacts: Test cases, code, build results.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Test Lead, Implementation Lead, Quality Lead.

## Required Links

Catalog required links: Test cases, code, build results.
...
**Example:** `Automated Test Evidence` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `execution logs` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Test Design

**Fill:** Specify levels, procedures, data, fixtures, negative cases, edge cases, and accessibility/security/NFR coverage.

**Example:** In CareQueue Pro, record how `CI run` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Execution Evidence

**Fill:** Record commands, logs, screenshots, reports, result interpretation, failures, and rerun instructions.

**Example:** In CareQueue Pro, record how `coverage` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Coverage and Gaps

**Fill:** Map results to requirements, risks, code, design, and residual gaps.

**Example:** In CareQueue Pro, record how `flake analysis` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Exit Decision

**Fill:** State pass/fail/conditional verdict, fixes, owner, revalidation, and certificate linkage.

**Example:** In CareQueue Pro, record how `failure triage` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Automated Test Evidence` focus: execution logs | TST-ACPT-019, REV-VNV-003-001 | Example only |
...
1. Artifact captures execution logs with owner, evidence, and trace links.
2. Artifact captures CI run with owner, evidence, and trace links.
3. Artifact captures coverage with owner, evidence, and trace links.
4. Artifact captures flake analysis with owner, evidence, and trace links.
5. Artifact captures failure triage with owner, evidence, and trace links.
...
1. VNV-003-C1 Test Evidence Critic: validates execution logs, pass/fail meaning, flake risk, and rerunability.
2. VNV-003-C2 Implementation Critic: validates test-code quality, fixtures, mocks, and maintenance burden.
3. VNV-003-C3 Quality Gate Critic: validates evidence links, failure triage, thresholds, and certificate readiness.
...
| Example review panel | `reviews/vnv-003-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: execution logs, CI run, coverage, flake analysis, failure triage.
- Example decision: accept this artifact only after the Test Lead, Implementation Lead, Quality Lead panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Automated Test Evidence` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `execution logs`, `CI run`, and `coverage` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `execution logs`.
