...
# VNV-005 Security Test Report Template

Status: template plus fictional worked example.

Template family: `verification-validation`.

Use this template when a DFMS factory run needs `Security Test Report` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.
...
| --- | --- |
| Artifact ID | VNV-005 |
| Artifact name | Security Test Report |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Security Test Report` evidence;
...
- Related artifacts: Threats, findings, mitigations.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Security Engineer, Test Lead, Governance Auditor.

## Required Links

Catalog required links: Threats, findings, mitigations.
...
**Example:** `Security Test Report` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `security test scope` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Test Design

**Fill:** Specify levels, procedures, data, fixtures, negative cases, edge cases, and accessibility/security/NFR coverage.

**Example:** In CareQueue Pro, record how `findings` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
**Example:** In CareQueue Pro, record how `retest` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Exit Decision

**Fill:** State pass/fail/conditional verdict, fixes, owner, revalidation, and certificate linkage.

**Example:** In CareQueue Pro, record how `residual risk` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Security Test Report` focus: security test scope | TST-ACPT-019, REV-VNV-005-001 | Example only |
...
1. Artifact captures security test scope with owner, evidence, and trace links.
2. Artifact captures findings with owner, evidence, and trace links.
3. Artifact captures severity with owner, evidence, and trace links.
4. Artifact captures retest with owner, evidence, and trace links.
5. Artifact captures residual risk with owner, evidence, and trace links.
...
1. VNV-005-C1 Security Finding Critic: validates test scope, findings, severity, exploitability, and mitigations.
2. VNV-005-C2 Verification Critic: validates reproduction, retest, evidence, and residual-risk closure.
3. VNV-005-C3 Governance Critic: validates acceptance owner, waiver policy, audit trail, and release implication.
...
| Example review panel | `reviews/vnv-005-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: security test scope, findings, severity, retest, residual risk.
- Example decision: accept this artifact only after the Security Engineer, Test Lead, Governance Auditor panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Security Test Report` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `security test scope`, `findings`, and `severity` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `security test scope`.
