...
# VNV-002 Test Cases and Procedures Template

Status: template plus fictional worked example.

Template family: `verification-validation`.

Use this template when a DFMS factory run needs `Test Cases and Procedures` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: ISO/IEEE-style test docs.
...
| --- | --- |
| Artifact ID | VNV-002 |
| Artifact name | Test Cases and Procedures |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Test Cases and Procedures` evidence;
...
- Related artifacts: Requirements, code paths, scenarios.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Test Lead, Domain Expert, Implementation Lead.

## Required Links

Catalog required links: Requirements, code paths, scenarios.
...
**Example:** `Test Cases and Procedures` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `preconditions` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Test Design

**Fill:** Specify levels, procedures, data, fixtures, negative cases, edge cases, and accessibility/security/NFR coverage.

**Example:** In CareQueue Pro, record how `test steps` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Execution Evidence

**Fill:** Record commands, logs, screenshots, reports, result interpretation, failures, and rerun instructions.

**Example:** In CareQueue Pro, record how `expected results` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Coverage and Gaps

**Fill:** Map results to requirements, risks, code, design, and residual gaps.

**Example:** In CareQueue Pro, record how `test data` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Exit Decision

**Fill:** State pass/fail/conditional verdict, fixes, owner, revalidation, and certificate linkage.

**Example:** In CareQueue Pro, record how `rerun notes` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Test Cases and Procedures` focus: preconditions | TST-ACPT-019, REV-VNV-002-001 | Example only |
...
1. Artifact captures preconditions with owner, evidence, and trace links.
2. Artifact captures test steps with owner, evidence, and trace links.
3. Artifact captures expected results with owner, evidence, and trace links.
4. Artifact captures test data with owner, evidence, and trace links.
5. Artifact captures rerun notes with owner, evidence, and trace links.
...
1. VNV-002-C1 Test Procedure Critic: validates steps, expected results, data, preconditions, and reproducibility.
2. VNV-002-C2 Domain Scenario Critic: validates real-world cases, business rules, and edge behavior.
3. VNV-002-C3 Implementation Coverage Critic: validates code-path mapping, automation fit, and regression scope.
...
| Example review panel | `reviews/vnv-002-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: preconditions, test steps, expected results, test data, rerun notes.
- Example decision: accept this artifact only after the Test Lead, Domain Expert, Implementation Lead panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Test Cases and Procedures` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `preconditions`, `test steps`, and `expected results` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `preconditions`.
