...
# VNV-004 Holdout Scenario Report Template

Status: template plus fictional worked example.

Template family: `verification-validation`.

Use this template when a DFMS factory run needs `Holdout Scenario Report` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: Dark factory validation.
...
| --- | --- |
| Artifact ID | VNV-004 |
| Artifact name | Holdout Scenario Report |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Holdout Scenario Report` evidence;
...
- Related artifacts: Acceptance criteria, runtime logs, scores.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: QA Lead, Product Expert, Governance Auditor.

## Required Links

Catalog required links: Acceptance criteria, runtime logs, scores.
...
**Example:** `Holdout Scenario Report` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `hidden scenario` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Test Design

**Fill:** Specify levels, procedures, data, fixtures, negative cases, edge cases, and accessibility/security/NFR coverage.

**Example:** In CareQueue Pro, record how `anti-gaming` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Execution Evidence

**Fill:** Record commands, logs, screenshots, reports, result interpretation, failures, and rerun instructions.

**Example:** In CareQueue Pro, record how `transfer test` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Coverage and Gaps

**Fill:** Map results to requirements, risks, code, design, and residual gaps.

**Example:** In CareQueue Pro, record how `score` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Exit Decision

**Fill:** State pass/fail/conditional verdict, fixes, owner, revalidation, and certificate linkage.

**Example:** In CareQueue Pro, record how `claim supported` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Holdout Scenario Report` focus: hidden scenario | TST-ACPT-019, REV-VNV-004-001 | Example only |
...
1. Artifact captures hidden scenario with owner, evidence, and trace links.
2. Artifact captures anti-gaming with owner, evidence, and trace links.
3. Artifact captures transfer test with owner, evidence, and trace links.
4. Artifact captures score with owner, evidence, and trace links.
5. Artifact captures claim supported with owner, evidence, and trace links.
...
1. VNV-004-C1 Holdout Design Critic: validates hidden scenarios, anti-gaming, transfer value, and difficulty.
2. VNV-004-C2 Product Reality Critic: validates realistic user workflows, edge cases, and adoption risk.
3. VNV-004-C3 Governance Evidence Critic: validates logs, scores, methodology, and trace to claims.
...
| Example review panel | `reviews/vnv-004-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: hidden scenario, anti-gaming, transfer test, score, claim supported.
- Example decision: accept this artifact only after the QA Lead, Product Expert, Governance Auditor panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Holdout Scenario Report` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `hidden scenario`, `anti-gaming`, and `transfer test` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `hidden scenario`.
