...
# REQ-006 Assumption and Constraint Log Template

Status: template plus fictional worked example.

Template family: `requirements-product`.

Use this template when a DFMS factory run needs `Assumption and Constraint Log` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: Professional SDLC handoff.
...
| --- | --- |
| Artifact ID | REQ-006 |
| Artifact name | Assumption and Constraint Log |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Assumption and Constraint Log` evidence;
...
- Related artifacts: Decisions, risks, open questions.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Requirements Lead, Delivery Manager, Human Owner.

## Required Links

Catalog required links: Decisions, risks, open questions.
...
**Example:** `Assumption and Constraint Log` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `assumption owner` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Requirement Decomposition

**Fill:** Break the artifact into capabilities, scenarios, NFRs, edge cases, and independently testable leaves.

**Example:** In CareQueue Pro, record how `confidence level` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Acceptance Model

**Fill:** Define acceptance criteria, examples, negative cases, holdouts, and transfer tests.

**Example:** In CareQueue Pro, record how `constraint type` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
**Example:** In CareQueue Pro, record how `decision impact` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Downstream Allocation

**Fill:** Map requirements to design, tests, operations, support, and project-book records.

**Example:** In CareQueue Pro, record how `revalidation trigger` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Assumption and Constraint Log` focus: assumption owner | TST-ACPT-019, REV-REQ-006-001 | Example only |
...
1. Artifact captures assumption owner with owner, evidence, and trace links.
2. Artifact captures confidence level with owner, evidence, and trace links.
3. Artifact captures constraint type with owner, evidence, and trace links.
4. Artifact captures decision impact with owner, evidence, and trace links.
5. Artifact captures revalidation trigger with owner, evidence, and trace links.
...
1. REQ-006-C1 Assumption Quality Critic: validates explicit assumptions, constraints, confidence, and decision impact.
2. REQ-006-C2 Delivery Governance Critic: validates owners, approvals, revalidation triggers, and change-control links.
3. REQ-006-C3 Human Decision Critic: validates unresolved questions, escalation, residual risk, and handoff visibility.
...
| Example review panel | `reviews/req-006-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: assumption owner, confidence level, constraint type, decision impact, revalidation trigger.
- Example decision: accept this artifact only after the Requirements Lead, Delivery Manager, Human Owner panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Assumption and Constraint Log` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `assumption owner`, `confidence level`, and `constraint type` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `assumption owner`.
