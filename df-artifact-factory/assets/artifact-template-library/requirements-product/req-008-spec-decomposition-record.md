...
# REQ-008 Spec Decomposition Record Template

Status: template plus fictional worked example.

Template family: `requirements-product`.

Use this template when a DFMS factory run needs `Spec Decomposition Record` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: ISO/IEC/IEEE 29148, RUP use-case decomposition, Agile story slicing, DDD domain decomposition.
...
| --- | --- |
| Artifact ID | REQ-008 |
| Artifact name | Spec Decomposition Record |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Spec Decomposition Record` evidence;
...
- Related artifacts: Requirement tree, branch interviews, completeness scores, leaf acceptance criteria, trace links, re-interrogation triggers.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Requirements Decomposition Lead, Product Owner, Verification Lead.

## Required Links

Catalog required links: Requirement tree, branch interviews, completeness scores, leaf acceptance criteria, trace links, re-interrogation triggers.
...
**Example:** `Spec Decomposition Record` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
### Customer Intent Source

**Fill:** Link every requirement branch to interview answer IDs, approved assumptions, or source transcript passages.

**Example:** In CareQueue Pro, record how `decomposition tree` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Requirement Decomposition

**Fill:** Break the artifact into capabilities, scenarios, NFRs, edge cases, and independently testable leaves.

**Example:** In CareQueue Pro, record how `parent-child coverage` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Acceptance Model

**Fill:** Define acceptance criteria, examples, negative cases, holdouts, and transfer tests.

**Example:** In CareQueue Pro, record how `atomic leaves` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Conflict and Completeness Handling

**Fill:** Record contradictions, completeness score, re-interrogation triggers, and customer approvals.

**Example:** In CareQueue Pro, record how `completeness score` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Downstream Allocation

**Fill:** Map requirements to design, tests, operations, support, and project-book records.

**Example:** In CareQueue Pro, record how `leaf acceptance tests` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Spec Decomposition Record` focus: decomposition tree | TST-ACPT-019, REV-REQ-008-001 | Example only |
...
1. Artifact captures decomposition tree with owner, evidence, and trace links.
2. Artifact captures parent-child coverage with owner, evidence, and trace links.
3. Artifact captures atomic leaves with owner, evidence, and trace links.
4. Artifact captures completeness score with owner, evidence, and trace links.
5. Artifact captures leaf acceptance tests with owner, evidence, and trace links.
...
1. REQ-008-C1 Decomposition Completeness Critic: validates parent-child coverage, atomic leaves, branch gaps, and recursion depth.
2. REQ-008-C2 Product Intent Critic: validates that decomposition preserves customer outcomes and avoids overfitting.
3. REQ-008-C3 Verification Mapping Critic: validates leaf acceptance criteria, holdouts, transfer tests, and trace closure.
...
| Example review panel | `reviews/req-008-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: decomposition tree, parent-child coverage, atomic leaves, completeness score, leaf acceptance tests.
- Example decision: accept this artifact only after the Requirements Decomposition Lead, Product Owner, Verification Lead panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Spec Decomposition Record` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `decomposition tree`, `parent-child coverage`, and `atomic leaves` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `decomposition tree`.
