...
# REQ-004 Acceptance Criteria and Scenarios Template

Status: template plus fictional worked example.

Template family: `requirements-product`.

Use this template when a DFMS factory run needs `Acceptance Criteria and Scenarios` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: BDD/TDD, dark factory validation.
...
| --- | --- |
| Artifact ID | REQ-004 |
| Artifact name | Acceptance Criteria and Scenarios |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Acceptance Criteria and Scenarios` evidence;
...
- Related artifacts: Requirements, tests, holdouts, user journeys.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Product Expert, QA Lead, Domain Expert.

## Required Links

Catalog required links: Requirements, tests, holdouts, user journeys.
...
**Example:** `Acceptance Criteria and Scenarios` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `user scenarios` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Requirement Decomposition

**Fill:** Break the artifact into capabilities, scenarios, NFRs, edge cases, and independently testable leaves.

**Example:** In CareQueue Pro, record how `acceptance criteria` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Acceptance Model

**Fill:** Define acceptance criteria, examples, negative cases, holdouts, and transfer tests.

**Example:** In CareQueue Pro, record how `negative cases` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
**Example:** In CareQueue Pro, record how `holdouts` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Downstream Allocation

**Fill:** Map requirements to design, tests, operations, support, and project-book records.

**Example:** In CareQueue Pro, record how `journey coverage` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Acceptance Criteria and Scenarios` focus: user scenarios | TST-ACPT-019, REV-REQ-004-001 | Example only |
...
1. Artifact captures user scenarios with owner, evidence, and trace links.
2. Artifact captures acceptance criteria with owner, evidence, and trace links.
3. Artifact captures negative cases with owner, evidence, and trace links.
4. Artifact captures holdouts with owner, evidence, and trace links.
5. Artifact captures journey coverage with owner, evidence, and trace links.
...
1. REQ-004-C1 Product Scenario Critic: validates user goals, business rules, edge scenarios, and prioritization.
2. REQ-004-C2 Domain Behavior Critic: validates domain language, workflows, invariants, and failure cases.
3. REQ-004-C3 BDD Verification Critic: validates given/when/then clarity, testability, holdouts, and regression scope.
...
| Example review panel | `reviews/req-004-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: user scenarios, acceptance criteria, negative cases, holdouts, journey coverage.
- Example decision: accept this artifact only after the Product Expert, QA Lead, Domain Expert panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Acceptance Criteria and Scenarios` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `user scenarios`, `acceptance criteria`, and `negative cases` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `user scenarios`.
