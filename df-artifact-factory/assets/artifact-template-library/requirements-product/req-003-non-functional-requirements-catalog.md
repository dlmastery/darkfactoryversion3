...
# REQ-003 Non-Functional Requirements Catalog Template

Status: template plus fictional worked example.

Template family: `requirements-product`.

Use this template when a DFMS factory run needs `Non-Functional Requirements Catalog` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: ISO quality, SRE, security.
...
| --- | --- |
| Artifact ID | REQ-003 |
| Artifact name | Non-Functional Requirements Catalog |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Non-Functional Requirements Catalog` evidence;
...
- Related artifacts: SLIs/SLOs, threat model, tests, operations.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: SRE Lead, Security Engineer, System Architect.

## Required Links

Catalog required links: SLIs/SLOs, threat model, tests, operations.
...
**Example:** `Non-Functional Requirements Catalog` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `availability` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Requirement Decomposition

**Fill:** Break the artifact into capabilities, scenarios, NFRs, edge cases, and independently testable leaves.

**Example:** In CareQueue Pro, record how `performance` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Acceptance Model

**Fill:** Define acceptance criteria, examples, negative cases, holdouts, and transfer tests.

**Example:** In CareQueue Pro, record how `security` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
**Example:** In CareQueue Pro, record how `privacy` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Downstream Allocation

**Fill:** Map requirements to design, tests, operations, support, and project-book records.

**Example:** In CareQueue Pro, record how `operability thresholds` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Non-Functional Requirements Catalog` focus: availability | TST-ACPT-019, REV-REQ-003-001 | Example only |
...
1. Artifact captures availability with owner, evidence, and trace links.
2. Artifact captures performance with owner, evidence, and trace links.
3. Artifact captures security with owner, evidence, and trace links.
4. Artifact captures privacy with owner, evidence, and trace links.
5. Artifact captures operability thresholds with owner, evidence, and trace links.
...
1. REQ-003-C1 Reliability and Operations Critic: validates SLIs/SLOs, capacity, availability, support, and observability needs.
2. REQ-003-C2 Security and Privacy NFR Critic: validates security, privacy, abuse, compliance, and data requirements.
3. REQ-003-C3 Architecture Feasibility Critic: validates tradeoffs, constraints, measurable thresholds, and test strategy.
...
| Example review panel | `reviews/req-003-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: availability, performance, security, privacy, operability thresholds.
- Example decision: accept this artifact only after the SRE Lead, Security Engineer, System Architect panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Non-Functional Requirements Catalog` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `availability`, `performance`, and `security` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `availability`.
