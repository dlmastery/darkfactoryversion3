...
# REQ-005 Glossary and Ubiquitous Language Template

Status: template plus fictional worked example.

Template family: `requirements-product`.

Use this template when a DFMS factory run needs `Glossary and Ubiquitous Language` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: DDD.
...
| --- | --- |
| Artifact ID | REQ-005 |
| Artifact name | Glossary and Ubiquitous Language |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Glossary and Ubiquitous Language` evidence;
...
- Related artifacts: Domain terms, bounded contexts, APIs.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Domain Expert, DDD Critic, Requirements Lead.

## Required Links

Catalog required links: Domain terms, bounded contexts, APIs.
...
**Example:** `Glossary and Ubiquitous Language` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `term definitions` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Requirement Decomposition

**Fill:** Break the artifact into capabilities, scenarios, NFRs, edge cases, and independently testable leaves.

**Example:** In CareQueue Pro, record how `bounded-context meaning` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Acceptance Model

**Fill:** Define acceptance criteria, examples, negative cases, holdouts, and transfer tests.

**Example:** In CareQueue Pro, record how `aliases` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
**Example:** In CareQueue Pro, record how `forbidden synonyms` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Downstream Allocation

**Fill:** Map requirements to design, tests, operations, support, and project-book records.

**Example:** In CareQueue Pro, record how `code/test naming links` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Glossary and Ubiquitous Language` focus: term definitions | TST-ACPT-019, REV-REQ-005-001 | Example only |
...
1. Artifact captures term definitions with owner, evidence, and trace links.
2. Artifact captures bounded-context meaning with owner, evidence, and trace links.
3. Artifact captures aliases with owner, evidence, and trace links.
4. Artifact captures forbidden synonyms with owner, evidence, and trace links.
5. Artifact captures code/test naming links with owner, evidence, and trace links.
...
1. REQ-005-C1 Domain Language Critic: validates terms, meanings, aliases, and stakeholder usage.
2. REQ-005-C2 DDD Context Critic: validates context-specific meanings, boundaries, aggregates, and translation points.
3. REQ-005-C3 Requirements Consistency Critic: validates term use across requirements, APIs, tests, and artifacts.
...
| Example review panel | `reviews/req-005-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: term definitions, bounded-context meaning, aliases, forbidden synonyms, code/test naming links.
- Example decision: accept this artifact only after the Domain Expert, DDD Critic, Requirements Lead panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Glossary and Ubiquitous Language` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `term definitions`, `bounded-context meaning`, and `aliases` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `term definitions`.
