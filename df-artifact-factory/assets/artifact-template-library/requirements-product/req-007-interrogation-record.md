...
# REQ-007 Interrogation Record Template

Status: template plus fictional worked example.

Template family: `requirements-product`.

Use this template when a DFMS factory run needs `Interrogation Record` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: Customer discovery and validation.
...
| --- | --- |
| Artifact ID | REQ-007 |
| Artifact name | Interrogation Record |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Interrogation Record` evidence;
...
- Related artifacts: Rounds, answer IDs, contradiction scoring, approval mechanics.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Requirements Lead, Product Owner, Governance Auditor.

## Required Links

Catalog required links: Rounds, answer IDs, contradiction scoring, approval mechanics.
...
**Example:** `Interrogation Record` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `interview rounds` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Requirement Decomposition

**Fill:** Break the artifact into capabilities, scenarios, NFRs, edge cases, and independently testable leaves.

**Example:** In CareQueue Pro, record how `answer IDs` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Acceptance Model

**Fill:** Define acceptance criteria, examples, negative cases, holdouts, and transfer tests.

**Example:** In CareQueue Pro, record how `contradiction scoring` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
**Example:** In CareQueue Pro, record how `approval mechanics` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Downstream Allocation

**Fill:** Map requirements to design, tests, operations, support, and project-book records.

**Example:** In CareQueue Pro, record how `re-interrogation triggers` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Interrogation Record` focus: interview rounds | TST-ACPT-019, REV-REQ-007-001 | Example only |
...
1. Artifact captures interview rounds with owner, evidence, and trace links.
2. Artifact captures answer IDs with owner, evidence, and trace links.
3. Artifact captures contradiction scoring with owner, evidence, and trace links.
4. Artifact captures approval mechanics with owner, evidence, and trace links.
5. Artifact captures re-interrogation triggers with owner, evidence, and trace links.
...
1. REQ-007-C1 Interrogation Protocol Critic: validates rounds, answer IDs, contradiction checks, completeness scoring, and re-interrogation.
2. REQ-007-C2 Product Owner Alignment Critic: validates customer validation, approval mechanics, priorities, and unresolved intent.
3. REQ-007-C3 Governance Trace Critic: validates answer-to-requirement links, assumptions, decisions, and approval evidence.
...
| Example review panel | `reviews/req-007-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: interview rounds, answer IDs, contradiction scoring, approval mechanics, re-interrogation triggers.
- Example decision: accept this artifact only after the Requirements Lead, Product Owner, Governance Auditor panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Interrogation Record` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `interview rounds`, `answer IDs`, and `contradiction scoring` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `interview rounds`.
