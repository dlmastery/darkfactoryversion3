...
# REQ-001 Business Requirements Document Template

Status: template plus fictional worked example.

Template family: `requirements-product`.

Use this template when a DFMS factory run needs `Business Requirements Document` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: ISO information item, RUP business modeling.
...
| --- | --- |
| Artifact ID | REQ-001 |
| Artifact name | Business Requirements Document |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Business Requirements Document` evidence;
...
- Related artifacts: Stakeholders, capabilities, business outcomes.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Product Expert, Domain Expert, Requirements Lead.

## Required Links

Catalog required links: Stakeholders, capabilities, business outcomes.
...
**Example:** `Business Requirements Document` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `business outcomes` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Requirement Decomposition

**Fill:** Break the artifact into capabilities, scenarios, NFRs, edge cases, and independently testable leaves.

**Example:** In CareQueue Pro, record how `stakeholder needs` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Acceptance Model

**Fill:** Define acceptance criteria, examples, negative cases, holdouts, and transfer tests.

**Example:** In CareQueue Pro, record how `capability map` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
**Example:** In CareQueue Pro, record how `success metrics` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Downstream Allocation

**Fill:** Map requirements to design, tests, operations, support, and project-book records.

**Example:** In CareQueue Pro, record how `business constraints` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Business Requirements Document` focus: business outcomes | TST-ACPT-019, REV-REQ-001-001 | Example only |
...
1. Artifact captures business outcomes with owner, evidence, and trace links.
2. Artifact captures stakeholder needs with owner, evidence, and trace links.
3. Artifact captures capability map with owner, evidence, and trace links.
4. Artifact captures success metrics with owner, evidence, and trace links.
5. Artifact captures business constraints with owner, evidence, and trace links.
...
1. REQ-001-C1 Business Value Critic: validates outcomes, stakeholders, capabilities, success measures, and priorities.
2. REQ-001-C2 Domain Realism Critic: validates workflow truth, vocabulary, constraints, and market/domain fit.
3. REQ-001-C3 Requirements Trace Critic: validates source answers, assumptions, approval state, and downstream acceptance links.
...
| Example review panel | `reviews/req-001-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: business outcomes, stakeholder needs, capability map, success metrics, business constraints.
- Example decision: accept this artifact only after the Product Expert, Domain Expert, Requirements Lead panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Business Requirements Document` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `business outcomes`, `stakeholder needs`, and `capability map` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `business outcomes`.
