...
# GOV-001 Project Charter Template

Status: template plus fictional worked example.

Template family: `governance-management`.

Use this template when a DFMS factory run needs `Project Charter` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: RUP inception, ISO agreement/project planning.
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Project Charter` evidence;
...
- Related artifacts: Stakeholders, objectives, scope, constraints.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Product Expert, Governance Auditor, Delivery Manager.

## Required Links

Catalog required links: Stakeholders, objectives, scope, constraints.
...
**Example:** `Project Charter` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `business case boundaries` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Scope and Tailoring

**Fill:** Define included/excluded lifecycle areas, method tailoring, waivers, and residual-risk ownership.

**Example:** In CareQueue Pro, record how `stakeholder map` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Authority Model

**Fill:** Name client owner, dark-factory delivery owner, approvers, reviewers, escalation path, and reapproval triggers.

**Example:** In CareQueue Pro, record how `success measures` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Token and Change Impact

**Fill:** Capture token SWAG, iteration boundary, budget assumptions, and change-control thresholds.

**Example:** In CareQueue Pro, record how `scope exclusions` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Governance Evidence

**Fill:** List records, approvals, control graph nodes, ledger items, certificates, and audit links.

**Example:** In CareQueue Pro, record how `project viability decision` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Project Charter` focus: business case boundaries | TST-ACPT-019, REV-GOV-001-001 | Example only |
...
1. Artifact captures business case boundaries with owner, evidence, and trace links.
2. Artifact captures stakeholder map with owner, evidence, and trace links.
3. Artifact captures success measures with owner, evidence, and trace links.
4. Artifact captures scope exclusions with owner, evidence, and trace links.
5. Artifact captures project viability decision with owner, evidence, and trace links.
...
1. GOV-001-C1 Strategic Charter Critic: validates objectives, stakeholders, value, boundaries, assumptions, and success criteria.
2. GOV-001-C2 Governance Baseline Critic: validates agreement basis, authority, approvals, standards tailoring link, and scope trace.
3. GOV-001-C3 Delivery Commitment Critic: validates delivery feasibility, checkpoint cadence, token-budget assumptions, and handoff clarity.
...
| Example review panel | `reviews/gov-001-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: business case boundaries, stakeholder map, success measures, scope exclusions, project viability decision.
- Example decision: accept this artifact only after the Product Expert, Governance Auditor, Delivery Manager panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Project Charter` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `business case boundaries`, `stakeholder map`, and `success measures` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `business case boundaries`.
