...
# GOV-004 Risk Register Template

Status: template plus fictional worked example.

Template family: `governance-management`.

Use this template when a DFMS factory run needs `Risk Register` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: ISO risk management, CMMI.
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Risk Register` evidence;
...
- Related artifacts: Requirements, decisions, tests, owners.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Delivery Manager, Security Engineer, System Architect.

## Required Links

Catalog required links: Requirements, decisions, tests, owners.
...
**Example:** `Risk Register` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `risk triggers` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Scope and Tailoring

**Fill:** Define included/excluded lifecycle areas, method tailoring, waivers, and residual-risk ownership.

**Example:** In CareQueue Pro, record how `probability and impact` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Authority Model

**Fill:** Name client owner, dark-factory delivery owner, approvers, reviewers, escalation path, and reapproval triggers.

**Example:** In CareQueue Pro, record how `mitigation owner` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Token and Change Impact

**Fill:** Capture token SWAG, iteration boundary, budget assumptions, and change-control thresholds.

**Example:** In CareQueue Pro, record how `contingency` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Governance Evidence

**Fill:** List records, approvals, control graph nodes, ledger items, certificates, and audit links.

**Example:** In CareQueue Pro, record how `residual-risk acceptance` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Risk Register` focus: risk triggers | TST-ACPT-019, REV-GOV-004-001 | Example only |
...
1. Artifact captures risk triggers with owner, evidence, and trace links.
2. Artifact captures probability and impact with owner, evidence, and trace links.
3. Artifact captures mitigation owner with owner, evidence, and trace links.
4. Artifact captures contingency with owner, evidence, and trace links.
5. Artifact captures residual-risk acceptance with owner, evidence, and trace links.
...
1. GOV-004-C1 Delivery Risk Critic: validates risk taxonomy, probability/impact, triggers, mitigation, and owners.
2. GOV-004-C2 Security and Compliance Risk Critic: validates security, privacy, AI, legal, and audit risks.
3. GOV-004-C3 Architecture Failure-Mode Critic: validates technical failure modes, systemic coupling, and verification coverage.
...
| Example review panel | `reviews/gov-004-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: risk triggers, probability and impact, mitigation owner, contingency, residual-risk acceptance.
- Example decision: accept this artifact only after the Delivery Manager, Security Engineer, System Architect panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Risk Register` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `risk triggers`, `probability and impact`, and `mitigation owner` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `risk triggers`.
