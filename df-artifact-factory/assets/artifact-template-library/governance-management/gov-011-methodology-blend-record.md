...
# GOV-011 Methodology Blend Record Template

Status: template plus fictional worked example.

Template family: `governance-management`.

Use this template when a DFMS factory run needs `Methodology Blend Record` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: RUP/MDA/DDD/TDD/SRE compiler.
...
| --- | --- |
| Artifact ID | GOV-011 |
| Artifact name | Methodology Blend Record |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Methodology Blend Record` evidence;
...
- Related artifacts: Included methods, graph nodes, ledger items, gates, evidence.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Methodology Architect, Governance Auditor, Quality Lead.

## Required Links

Catalog required links: Included methods, graph nodes, ledger items, gates, evidence.
...
**Example:** `Methodology Blend Record` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `method mix` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Scope and Tailoring

**Fill:** Define included/excluded lifecycle areas, method tailoring, waivers, and residual-risk ownership.

**Example:** In CareQueue Pro, record how `method conflicts` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Authority Model

**Fill:** Name client owner, dark-factory delivery owner, approvers, reviewers, escalation path, and reapproval triggers.

**Example:** In CareQueue Pro, record how `graph compilation` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Token and Change Impact

**Fill:** Capture token SWAG, iteration boundary, budget assumptions, and change-control thresholds.

**Example:** In CareQueue Pro, record how `method-specific gates` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Governance Evidence

**Fill:** List records, approvals, control graph nodes, ledger items, certificates, and audit links.

**Example:** In CareQueue Pro, record how `evidence commitments` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Methodology Blend Record` focus: method mix | TST-ACPT-019, REV-GOV-011-001 | Example only |
...
1. Artifact captures method mix with owner, evidence, and trace links.
2. Artifact captures method conflicts with owner, evidence, and trace links.
3. Artifact captures graph compilation with owner, evidence, and trace links.
4. Artifact captures method-specific gates with owner, evidence, and trace links.
5. Artifact captures evidence commitments with owner, evidence, and trace links.
...
1. GOV-011-C1 Methodology Compiler Critic: validates RUP/MDA/DDD/TDD/SRE blend and conflict resolution.
2. GOV-011-C2 Governance Tailoring Critic: validates method-specific gates, artifacts, waivers, and standards rationale.
3. GOV-011-C3 Quality Evidence Critic: validates graph nodes, ledger items, transformation records, and refinery evidence.
...
| Example review panel | `reviews/gov-011-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: method mix, method conflicts, graph compilation, method-specific gates, evidence commitments.
- Example decision: accept this artifact only after the Methodology Architect, Governance Auditor, Quality Lead panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Methodology Blend Record` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `method mix`, `method conflicts`, and `graph compilation` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `method mix`.
