...
# GOV-002 Standards Tailoring Matrix Template

Status: template plus fictional worked example.

Template family: `governance-management`.

Use this template when a DFMS factory run needs `Standards Tailoring Matrix` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: ISO 12207, ISO 15289, company process.
...
| --- | --- |
| Artifact ID | GOV-002 |
| Artifact name | Standards Tailoring Matrix |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Standards Tailoring Matrix` evidence;
...
- Related artifacts: Standards clauses, included/excluded artifacts, rationale.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Governance Auditor, Quality Lead, Human Owner.

## Required Links

Catalog required links: Standards clauses, included/excluded artifacts, rationale.
...
**Example:** `Standards Tailoring Matrix` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `included standards` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Scope and Tailoring

**Fill:** Define included/excluded lifecycle areas, method tailoring, waivers, and residual-risk ownership.

**Example:** In CareQueue Pro, record how `excluded standards` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Authority Model

**Fill:** Name client owner, dark-factory delivery owner, approvers, reviewers, escalation path, and reapproval triggers.

**Example:** In CareQueue Pro, record how `tailoring rationale` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Token and Change Impact

**Fill:** Capture token SWAG, iteration boundary, budget assumptions, and change-control thresholds.

**Example:** In CareQueue Pro, record how `information-item mapping` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Governance Evidence

**Fill:** List records, approvals, control graph nodes, ledger items, certificates, and audit links.

**Example:** In CareQueue Pro, record how `waiver policy` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Standards Tailoring Matrix` focus: included standards | TST-ACPT-019, REV-GOV-002-001 | Example only |
...
1. Artifact captures included standards with owner, evidence, and trace links.
2. Artifact captures excluded standards with owner, evidence, and trace links.
3. Artifact captures tailoring rationale with owner, evidence, and trace links.
4. Artifact captures information-item mapping with owner, evidence, and trace links.
5. Artifact captures waiver policy with owner, evidence, and trace links.
...
1. GOV-002-C1 Standards Tailoring Critic: validates included/excluded standards, method fit, and tailoring rationale.
2. GOV-002-C2 Audit Evidence Critic: validates clauses, information items, waivers, owners, and defensible compliance language.
3. GOV-002-C3 Quality Threshold Critic: validates quality gates, artifact coverage, review thresholds, and certificate implications.
...
| Example review panel | `reviews/gov-002-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: included standards, excluded standards, tailoring rationale, information-item mapping, waiver policy.
- Example decision: accept this artifact only after the Governance Auditor, Quality Lead, Human Owner panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Standards Tailoring Matrix` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `included standards`, `excluded standards`, and `tailoring rationale` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `included standards`.
