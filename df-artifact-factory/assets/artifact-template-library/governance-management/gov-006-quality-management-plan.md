...
# GOV-006 Quality Management Plan Template

Status: template plus fictional worked example.

Template family: `governance-management`.

Use this template when a DFMS factory run needs `Quality Management Plan` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: ISO 12207, CMMI.
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Quality Management Plan` evidence;
...
- Related artifacts: Rubrics, thresholds, evidence types.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Quality Lead, Governance Auditor, Test Lead.

## Required Links

Catalog required links: Rubrics, thresholds, evidence types.
...
**Example:** `Quality Management Plan` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `quality thresholds` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Scope and Tailoring

**Fill:** Define included/excluded lifecycle areas, method tailoring, waivers, and residual-risk ownership.

**Example:** In CareQueue Pro, record how `rubric model` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Authority Model

**Fill:** Name client owner, dark-factory delivery owner, approvers, reviewers, escalation path, and reapproval triggers.

**Example:** In CareQueue Pro, record how `review cadence` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Token and Change Impact

**Fill:** Capture token SWAG, iteration boundary, budget assumptions, and change-control thresholds.

**Example:** In CareQueue Pro, record how `certificate policy` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Governance Evidence

**Fill:** List records, approvals, control graph nodes, ledger items, certificates, and audit links.

**Example:** In CareQueue Pro, record how `defect escape handling` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Quality Management Plan` focus: quality thresholds | TST-ACPT-019, REV-GOV-006-001 | Example only |
...
1. Artifact captures quality thresholds with owner, evidence, and trace links.
2. Artifact captures rubric model with owner, evidence, and trace links.
3. Artifact captures review cadence with owner, evidence, and trace links.
4. Artifact captures certificate policy with owner, evidence, and trace links.
5. Artifact captures defect escape handling with owner, evidence, and trace links.
...
1. GOV-006-C1 Quality System Critic: validates rubric model, thresholds, defect loops, and acceptance gates.
2. GOV-006-C2 Governance Audit Critic: validates standards mapping, certificate policy, evidence sufficiency, and waiver handling.
3. GOV-006-C3 Verification Strategy Critic: validates test levels, holdouts, rerunability, regression, and exit criteria.
...
| Example review panel | `reviews/gov-006-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: quality thresholds, rubric model, review cadence, certificate policy, defect escape handling.
- Example decision: accept this artifact only after the Quality Lead, Governance Auditor, Test Lead panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Quality Management Plan` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `quality thresholds`, `rubric model`, and `review cadence` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `quality thresholds`.
