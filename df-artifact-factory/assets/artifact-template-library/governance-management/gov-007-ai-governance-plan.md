...
# GOV-007 AI Governance Plan Template

Status: template plus fictional worked example.

Template family: `governance-management`.

Use this template when a DFMS factory run needs `AI Governance Plan` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: ISO 42001.
...
| --- | --- |
| Artifact ID | GOV-007 |
| Artifact name | AI Governance Plan |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `AI Governance Plan` evidence;
...
- Related artifacts: Agent roles, model use, review gates, monitoring.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: AI Governance Auditor, Security Engineer, Human Owner.

## Required Links

Catalog required links: Agent roles, model use, review gates, monitoring.
...
**Example:** `AI Governance Plan` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `agent authority` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Scope and Tailoring

**Fill:** Define included/excluded lifecycle areas, method tailoring, waivers, and residual-risk ownership.

**Example:** In CareQueue Pro, record how `model-use policy` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Authority Model

**Fill:** Name client owner, dark-factory delivery owner, approvers, reviewers, escalation path, and reapproval triggers.

**Example:** In CareQueue Pro, record how `data exposure` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Token and Change Impact

**Fill:** Capture token SWAG, iteration boundary, budget assumptions, and change-control thresholds.

**Example:** In CareQueue Pro, record how `human override` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Governance Evidence

**Fill:** List records, approvals, control graph nodes, ledger items, certificates, and audit links.

**Example:** In CareQueue Pro, record how `monitoring and audit` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `AI Governance Plan` focus: agent authority | TST-ACPT-019, REV-GOV-007-001 | Example only |
...
1. Artifact captures agent authority with owner, evidence, and trace links.
2. Artifact captures model-use policy with owner, evidence, and trace links.
3. Artifact captures data exposure with owner, evidence, and trace links.
4. Artifact captures human override with owner, evidence, and trace links.
5. Artifact captures monitoring and audit with owner, evidence, and trace links.
...
1. GOV-007-C1 AI Governance Critic: validates agent authority, model use, review gates, policy boundaries, and monitoring.
2. GOV-007-C2 Security and Privacy Critic: validates data classification, leakage risk, access control, and abuse cases.
3. GOV-007-C3 Human Oversight Critic: validates human decision points, override paths, audit trail, and residual-risk acceptance.
...
| Example review panel | `reviews/gov-007-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: agent authority, model-use policy, data exposure, human override, monitoring and audit.
- Example decision: accept this artifact only after the AI Governance Auditor, Security Engineer, Human Owner panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `AI Governance Plan` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `agent authority`, `model-use policy`, and `data exposure` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `agent authority`.
