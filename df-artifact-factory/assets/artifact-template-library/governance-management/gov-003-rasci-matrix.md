...
# GOV-003 RASCI Matrix Template

Status: template plus fictional worked example.

Template family: `governance-management`.

Use this template when a DFMS factory run needs `RASCI Matrix` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: Human-agent accountability.
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `RASCI Matrix` evidence;
...
- Related artifacts: Roles, stages, approval authority.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Governance Auditor, Delivery Manager, Human Owner.

## Required Links

Catalog required links: Roles, stages, approval authority.
...
**Example:** `RASCI Matrix` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `accountable owners` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Scope and Tailoring

**Fill:** Define included/excluded lifecycle areas, method tailoring, waivers, and residual-risk ownership.

**Example:** In CareQueue Pro, record how `approval authority` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Authority Model

**Fill:** Name client owner, dark-factory delivery owner, approvers, reviewers, escalation path, and reapproval triggers.

**Example:** In CareQueue Pro, record how `agent/human handoff` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Token and Change Impact

**Fill:** Capture token SWAG, iteration boundary, budget assumptions, and change-control thresholds.

**Example:** In CareQueue Pro, record how `segregation of duties` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Governance Evidence

**Fill:** List records, approvals, control graph nodes, ledger items, certificates, and audit links.

**Example:** In CareQueue Pro, record how `escalation path` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `RASCI Matrix` focus: accountable owners | TST-ACPT-019, REV-GOV-003-001 | Example only |
...
1. Artifact captures accountable owners with owner, evidence, and trace links.
2. Artifact captures approval authority with owner, evidence, and trace links.
3. Artifact captures agent/human handoff with owner, evidence, and trace links.
4. Artifact captures segregation of duties with owner, evidence, and trace links.
5. Artifact captures escalation path with owner, evidence, and trace links.
...
1. GOV-003-C1 Accountability Design Critic: validates role clarity, decision rights, and human-agent ownership.
2. GOV-003-C2 Governance Authority Critic: validates approval authority, segregation of duties, waiver owner, and escalation path.
3. GOV-003-C3 Delivery Continuity Critic: validates takeover, handback, async review, and operational continuity.
...
| Example review panel | `reviews/gov-003-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: accountable owners, approval authority, agent/human handoff, segregation of duties, escalation path.
- Example decision: accept this artifact only after the Governance Auditor, Delivery Manager, Human Owner panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `RASCI Matrix` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `accountable owners`, `approval authority`, and `agent/human handoff` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `accountable owners`.
