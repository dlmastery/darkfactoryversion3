...
# GOV-005 Change Management Plan Template

Status: template plus fictional worked example.

Template family: `governance-management`.

Use this template when a DFMS factory run needs `Change Management Plan` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: Agile/RUP/CMMI.
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Change Management Plan` evidence;
...
- Related artifacts: Change requests, sprints, releases, approvals.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Change Manager, Governance Auditor, Product Owner.

## Required Links

Catalog required links: Change requests, sprints, releases, approvals.
...
**Example:** `Change Management Plan` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `change classes` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Scope and Tailoring

**Fill:** Define included/excluded lifecycle areas, method tailoring, waivers, and residual-risk ownership.

**Example:** In CareQueue Pro, record how `impact analysis` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Authority Model

**Fill:** Name client owner, dark-factory delivery owner, approvers, reviewers, escalation path, and reapproval triggers.

**Example:** In CareQueue Pro, record how `approval route` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Token and Change Impact

**Fill:** Capture token SWAG, iteration boundary, budget assumptions, and change-control thresholds.

**Example:** In CareQueue Pro, record how `token re-estimation` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Governance Evidence

**Fill:** List records, approvals, control graph nodes, ledger items, certificates, and audit links.

**Example:** In CareQueue Pro, record how `baseline update` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Change Management Plan` focus: change classes | TST-ACPT-019, REV-GOV-005-001 | Example only |
...
1. Artifact captures change classes with owner, evidence, and trace links.
2. Artifact captures impact analysis with owner, evidence, and trace links.
3. Artifact captures approval route with owner, evidence, and trace links.
4. Artifact captures token re-estimation with owner, evidence, and trace links.
5. Artifact captures baseline update with owner, evidence, and trace links.
...
1. GOV-005-C1 Change Control Critic: validates change classes, intake, impact analysis, approvals, and rebaseline rules.
2. GOV-005-C2 Governance Trace Critic: validates change-to-requirement/test/artifact trace and audit history.
3. GOV-005-C3 Product and Client Impact Critic: validates user impact, communication, token-budget drift, and steering checkpoints.
...
| Example review panel | `reviews/gov-005-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: change classes, impact analysis, approval route, token re-estimation, baseline update.
- Example decision: accept this artifact only after the Change Manager, Governance Auditor, Product Owner panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Change Management Plan` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `change classes`, `impact analysis`, and `approval route` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `change classes`.
