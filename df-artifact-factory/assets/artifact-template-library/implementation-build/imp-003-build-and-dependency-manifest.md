...
# IMP-003 Build and Dependency Manifest Template

Status: template plus fictional worked example.

Template family: `implementation-build`.

Use this template when a DFMS factory run needs `Build and Dependency Manifest` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: SSDF, supply chain.
...
| --- | --- |
| Artifact ID | IMP-003 |
| Artifact name | Build and Dependency Manifest |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Build and Dependency Manifest` evidence;
...
- Related artifacts: SBOM, lockfiles, build logs.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Security Engineer, SRE Lead, Implementation Lead.

## Required Links

Catalog required links: SBOM, lockfiles, build logs.
...
**Example:** `Build and Dependency Manifest` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `dependencies` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Build and Configuration

**Fill:** Capture build path, environment assumptions, config, dependency policy, and automation.

**Example:** In CareQueue Pro, record how `lockfiles` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Secure Construction

**Fill:** Address input validation, secrets, auth, data handling, dependency risk, and abuse cases.

**Example:** In CareQueue Pro, record how `SBOM` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Test and Review Evidence

**Fill:** Map code to requirements, tests, static checks, CI logs, review findings, and fixes.

**Example:** In CareQueue Pro, record how `build logs` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Rollback and Maintainability

**Fill:** Record backout plan, safe-change guidance, known debt, and future maintainer notes.

**Example:** In CareQueue Pro, record how `license/security checks` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Build and Dependency Manifest` focus: dependencies | TST-ACPT-019, REV-IMP-003-001 | Example only |
...
1. Artifact captures dependencies with owner, evidence, and trace links.
2. Artifact captures lockfiles with owner, evidence, and trace links.
3. Artifact captures SBOM with owner, evidence, and trace links.
4. Artifact captures build logs with owner, evidence, and trace links.
5. Artifact captures license/security checks with owner, evidence, and trace links.
...
1. IMP-003-C1 Supply Chain Security Critic: validates dependencies, lockfiles, SBOM, provenance, and vulnerability posture.
2. IMP-003-C2 SRE Build Reliability Critic: validates reproducible builds, CI behavior, environment assumptions, and rollout impact.
3. IMP-003-C3 Implementation Dependency Critic: validates package fit, version constraints, license notes, and developer workflow.
...
| Example review panel | `reviews/imp-003-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: dependencies, lockfiles, SBOM, build logs, license/security checks.
- Example decision: accept this artifact only after the Security Engineer, SRE Lead, Implementation Lead panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Build and Dependency Manifest` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `dependencies`, `lockfiles`, and `SBOM` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `dependencies`.
