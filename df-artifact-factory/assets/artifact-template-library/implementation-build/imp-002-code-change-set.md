...
# IMP-002 Code Change Set Template

Status: template plus fictional worked example.

Template family: `implementation-build`.

Use this template when a DFMS factory run needs `Code Change Set` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: Source control.
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Code Change Set` evidence;
...
- Related artifacts: Requirements, designs, tests, reviews.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Implementation Lead, Test Lead, Security Engineer.

## Required Links

Catalog required links: Requirements, designs, tests, reviews.
...
**Example:** `Code Change Set` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `changed files` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Build and Configuration

**Fill:** Capture build path, environment assumptions, config, dependency policy, and automation.

**Example:** In CareQueue Pro, record how `behavior change` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Secure Construction

**Fill:** Address input validation, secrets, auth, data handling, dependency risk, and abuse cases.

**Example:** In CareQueue Pro, record how `tests` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Test and Review Evidence

**Fill:** Map code to requirements, tests, static checks, CI logs, review findings, and fixes.

**Example:** In CareQueue Pro, record how `review findings` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Rollback and Maintainability

**Fill:** Record backout plan, safe-change guidance, known debt, and future maintainer notes.

**Example:** In CareQueue Pro, record how `security impact` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Code Change Set` focus: changed files | TST-ACPT-019, REV-IMP-002-001 | Example only |
...
1. Artifact captures changed files with owner, evidence, and trace links.
2. Artifact captures behavior change with owner, evidence, and trace links.
3. Artifact captures tests with owner, evidence, and trace links.
4. Artifact captures review findings with owner, evidence, and trace links.
5. Artifact captures security impact with owner, evidence, and trace links.
...
1. IMP-002-C1 Code Quality Critic: validates correctness, maintainability, minimality, conventions, and readability.
2. IMP-002-C2 Test and Regression Critic: validates tests, failures, coverage, fixtures, and reproducibility.
3. IMP-002-C3 Security Critic: validates secure coding, dependency risk, secrets, auth, and data handling.
...
| Example review panel | `reviews/imp-002-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: changed files, behavior change, tests, review findings, security impact.
- Example decision: accept this artifact only after the Implementation Lead, Test Lead, Security Engineer panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Code Change Set` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `changed files`, `behavior change`, and `tests` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `changed files`.
