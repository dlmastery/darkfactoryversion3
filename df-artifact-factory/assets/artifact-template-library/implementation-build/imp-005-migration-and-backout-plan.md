...
# IMP-005 Migration and Backout Plan Template

Status: template plus fictional worked example.

Template family: `implementation-build`.

Use this template when a DFMS factory run needs `Migration and Backout Plan` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: Release/SRE.
...
| --- | --- |
| Artifact ID | IMP-005 |
| Artifact name | Migration and Backout Plan |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Migration and Backout Plan` evidence;
...
- Related artifacts: Data model, deployment, rollback.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Data Architect, SRE Lead, Test Lead.

## Required Links

Catalog required links: Data model, deployment, rollback.
...
**Example:** `Migration and Backout Plan` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `migration runbook` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Build and Configuration

**Fill:** Capture build path, environment assumptions, config, dependency policy, and automation.

**Example:** In CareQueue Pro, record how `dry run` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Secure Construction

**Fill:** Address input validation, secrets, auth, data handling, dependency risk, and abuse cases.

**Example:** In CareQueue Pro, record how `validation query` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Test and Review Evidence

**Fill:** Map code to requirements, tests, static checks, CI logs, review findings, and fixes.

**Example:** In CareQueue Pro, record how `rollback` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Rollback and Maintainability

**Fill:** Record backout plan, safe-change guidance, known debt, and future maintainer notes.

**Example:** In CareQueue Pro, record how `cutover window` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Migration and Backout Plan` focus: migration runbook | TST-ACPT-019, REV-IMP-005-001 | Example only |
...
1. Artifact captures migration runbook with owner, evidence, and trace links.
2. Artifact captures dry run with owner, evidence, and trace links.
3. Artifact captures validation query with owner, evidence, and trace links.
4. Artifact captures rollback with owner, evidence, and trace links.
5. Artifact captures cutover window with owner, evidence, and trace links.
...
1. IMP-005-C1 Data Migration Critic: validates migration steps, validation, backfill, data quality, and ownership.
2. IMP-005-C2 SRE Rollback Critic: validates backout, cutover, monitoring, timing, and blast-radius controls.
3. IMP-005-C3 Test Critic: validates dry run, test data, reconciliation, performance, and failure scenarios.
...
| Example review panel | `reviews/imp-005-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: migration runbook, dry run, validation query, rollback, cutover window.
- Example decision: accept this artifact only after the Data Architect, SRE Lead, Test Lead panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Migration and Backout Plan` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `migration runbook`, `dry run`, and `validation query` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `migration runbook`.
