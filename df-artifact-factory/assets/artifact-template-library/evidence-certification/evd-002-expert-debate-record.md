...
# EVD-002 Expert Debate Record Template

Status: template plus fictional worked example.

Template family: `evidence-certification`.

Use this template when a DFMS factory run needs `Expert Debate Record` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: DFMS governance.
...
| --- | --- |
| Artifact ID | EVD-002 |
| Artifact name | Expert Debate Record |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Expert Debate Record` evidence;
...
- Related artifacts: Proposals, critiques, votes, decision.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Moderator, Governance Auditor, Human Owner.

## Required Links

Catalog required links: Proposals, critiques, votes, decision.
...
**Example:** `Expert Debate Record` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `persona declarations` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Evidence Inventory

**Fill:** List source files, links, logs, decisions, reviewer records, tests, certificates, hashes, and approvals.

**Example:** In CareQueue Pro, record how `independent proposals` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Trace Closure

**Fill:** Show forward and reverse links across intent, requirements, design, code, tests, operations, and risks.

**Example:** In CareQueue Pro, record how `cross critiques` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Decision and Residual Risk

**Fill:** Record verdict, conditions, waivers, residual risk, owner, expiry, and revalidation trigger.

**Example:** In CareQueue Pro, record how `votes` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Replay and Auditability

**Fill:** Explain how a fresh reviewer can verify, rerun, or resume from this record.

**Example:** In CareQueue Pro, record how `dissent` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Expert Debate Record` focus: persona declarations | TST-ACPT-019, REV-EVD-002-001 | Example only |
...
1. Artifact captures persona declarations with owner, evidence, and trace links.
2. Artifact captures independent proposals with owner, evidence, and trace links.
3. Artifact captures cross critiques with owner, evidence, and trace links.
4. Artifact captures votes with owner, evidence, and trace links.
5. Artifact captures dissent with owner, evidence, and trace links.
...
1. EVD-002-C1 Panel Integrity Critic: validates persona declarations, independent proposals, dissent, and decision vote.
2. EVD-002-C2 Governance Critic: validates links to node, ledger, decision, trace, and refinery gate.
3. EVD-002-C3 Human Accountability Critic: validates escalation, approval owner, residual risk, and handoff note.
...
| Example review panel | `reviews/evd-002-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: persona declarations, independent proposals, cross critiques, votes, dissent.
- Example decision: accept this artifact only after the Moderator, Governance Auditor, Human Owner panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Expert Debate Record` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `persona declarations`, `independent proposals`, and `cross critiques` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `persona declarations`.
