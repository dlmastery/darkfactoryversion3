...
# EVD-008 Human Communication Record Template

Status: template plus fictional worked example.

Template family: `evidence-certification`.

Use this template when a DFMS factory run needs `Human Communication Record` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: Human-centered SDLC collaboration.
...
| --- | --- |
| Artifact ID | EVD-008 |
| Artifact name | Human Communication Record |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Human Communication Record` evidence;
...
- Related artifacts: Async comments, confidence framing, taste gates, approval, incorporation evidence.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Human Owner, Delivery Manager, Quality Lead.

## Required Links

Catalog required links: Async comments, confidence framing, taste gates, approval, incorporation evidence.
...
**Example:** `Human Communication Record` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `async comments` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Evidence Inventory

**Fill:** List source files, links, logs, decisions, reviewer records, tests, certificates, hashes, and approvals.

**Example:** In CareQueue Pro, record how `confidence framing` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Trace Closure

**Fill:** Show forward and reverse links across intent, requirements, design, code, tests, operations, and risks.

**Example:** In CareQueue Pro, record how `disagreement handling` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Decision and Residual Risk

**Fill:** Record verdict, conditions, waivers, residual risk, owner, expiry, and revalidation trigger.

**Example:** In CareQueue Pro, record how `feedback incorporation` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Replay and Auditability

**Fill:** Explain how a fresh reviewer can verify, rerun, or resume from this record.

**Example:** In CareQueue Pro, record how `approval` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Human Communication Record` focus: async comments | TST-ACPT-019, REV-EVD-008-001 | Example only |
...
1. Artifact captures async comments with owner, evidence, and trace links.
2. Artifact captures confidence framing with owner, evidence, and trace links.
3. Artifact captures disagreement handling with owner, evidence, and trace links.
4. Artifact captures feedback incorporation with owner, evidence, and trace links.
5. Artifact captures approval with owner, evidence, and trace links.
...
1. EVD-008-C1 Human Decision Critic: validates async comments, approvals, taste gates, and decision clarity.
2. EVD-008-C2 Delivery Communication Critic: validates confidence framing, disagreement handling, and incorporation path.
3. EVD-008-C3 Quality Incorporation Critic: validates feedback-to-change evidence, re-review, and closure.
...
| Example review panel | `reviews/evd-008-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: async comments, confidence framing, disagreement handling, feedback incorporation, approval.
- Example decision: accept this artifact only after the Human Owner, Delivery Manager, Quality Lead panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Human Communication Record` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `async comments`, `confidence framing`, and `disagreement handling` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `async comments`.
