...
# EVD-009 Context Pack and Predecessor Recovery Record Template

Status: template plus fictional worked example.

Template family: `evidence-certification`.

Use this template when a DFMS factory run needs `Context Pack and Predecessor Recovery Record` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: Gas Town-inspired context rot control.
...
| --- | --- |
| Artifact ID | EVD-009 |
| Artifact name | Context Pack and Predecessor Recovery Record |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Context Pack and Predecessor Recovery Record` evidence;
...
- Related artifacts: Current node, recovered decisions, drift, replay evidence, next action.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Maintainer, Governance Auditor, Human Owner.

## Required Links

Catalog required links: Current node, recovered decisions, drift, replay evidence, next action.
...
**Example:** `Context Pack and Predecessor Recovery Record` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `current node` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Evidence Inventory

**Fill:** List source files, links, logs, decisions, reviewer records, tests, certificates, hashes, and approvals.

**Example:** In CareQueue Pro, record how `predecessor decisions` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Trace Closure

**Fill:** Show forward and reverse links across intent, requirements, design, code, tests, operations, and risks.

**Example:** In CareQueue Pro, record how `evidence index` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Decision and Residual Risk

**Fill:** Record verdict, conditions, waivers, residual risk, owner, expiry, and revalidation trigger.

**Example:** In CareQueue Pro, record how `replay drill` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Replay and Auditability

**Fill:** Explain how a fresh reviewer can verify, rerun, or resume from this record.

**Example:** In CareQueue Pro, record how `next action` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Context Pack and Predecessor Recovery Record` focus: current node | TST-ACPT-019, REV-EVD-009-001 | Example only |
...
1. Artifact captures current node with owner, evidence, and trace links.
2. Artifact captures predecessor decisions with owner, evidence, and trace links.
3. Artifact captures evidence index with owner, evidence, and trace links.
4. Artifact captures replay drill with owner, evidence, and trace links.
5. Artifact captures next action with owner, evidence, and trace links.
...
1. EVD-009-C1 Continuity Critic: validates current node, next action, decisions, and resume path.
2. EVD-009-C2 Governance Memory Critic: validates evidence index, drift, trace links, and audit history.
3. EVD-009-C3 Human Recovery Critic: validates replay drill, predecessor state, and handoff usability.
...
| Example review panel | `reviews/evd-009-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: current node, predecessor decisions, evidence index, replay drill, next action.
- Example decision: accept this artifact only after the Maintainer, Governance Auditor, Human Owner panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Context Pack and Predecessor Recovery Record` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `current node`, `predecessor decisions`, and `evidence index` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `current node`.
