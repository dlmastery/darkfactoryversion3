...
# EVD-005 Provenance Record Template

Status: template plus fictional worked example.

Template family: `evidence-certification`.

Use this template when a DFMS factory run needs `Provenance Record` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: Supply chain/audit.
...
| --- | --- |
| Artifact ID | EVD-005 |
| Artifact name | Provenance Record |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Provenance Record` evidence;
...
- Related artifacts: Hashes, commits, logs, prompts if allowed.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Governance Auditor, Security Engineer, Release Manager.

## Required Links

Catalog required links: Hashes, commits, logs, prompts if allowed.
...
**Example:** `Provenance Record` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `hashes` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Evidence Inventory

**Fill:** List source files, links, logs, decisions, reviewer records, tests, certificates, hashes, and approvals.

**Example:** In CareQueue Pro, record how `commits` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Trace Closure

**Fill:** Show forward and reverse links across intent, requirements, design, code, tests, operations, and risks.

**Example:** In CareQueue Pro, record how `generation logs` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Decision and Residual Risk

**Fill:** Record verdict, conditions, waivers, residual risk, owner, expiry, and revalidation trigger.

**Example:** In CareQueue Pro, record how `source references` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Replay and Auditability

**Fill:** Explain how a fresh reviewer can verify, rerun, or resume from this record.

**Example:** In CareQueue Pro, record how `reproducibility` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Provenance Record` focus: hashes | TST-ACPT-019, REV-EVD-005-001 | Example only |
...
1. Artifact captures hashes with owner, evidence, and trace links.
2. Artifact captures commits with owner, evidence, and trace links.
3. Artifact captures generation logs with owner, evidence, and trace links.
4. Artifact captures source references with owner, evidence, and trace links.
5. Artifact captures reproducibility with owner, evidence, and trace links.
...
1. EVD-005-C1 Provenance Critic: validates hashes, commits, logs, source references, and reproducibility.
2. EVD-005-C2 Security Supply-Chain Critic: validates dependency/source trust, script safety, and artifact integrity.
3. EVD-005-C3 Release Audit Critic: validates version lineage, build/export path, and release trace.
...
| Example review panel | `reviews/evd-005-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: hashes, commits, generation logs, source references, reproducibility.
- Example decision: accept this artifact only after the Governance Auditor, Security Engineer, Release Manager panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Provenance Record` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `hashes`, `commits`, and `generation logs` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `hashes`.
