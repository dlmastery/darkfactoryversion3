...
# EVD-007 Refinery Gate Record Template

Status: template plus fictional worked example.

Template family: `evidence-certification`.

Use this template when a DFMS factory run needs `Refinery Gate Record` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: DFMS/Fabro/Gas Town verification merge.
...
| --- | --- |
| Artifact ID | EVD-007 |
| Artifact name | Refinery Gate Record |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Refinery Gate Record` evidence;
...
- Related artifacts: Tests, traceability, rubrics, security, operations, residual risk.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Quality Lead, Test Lead, Governance Auditor.

## Required Links

Catalog required links: Tests, traceability, rubrics, security, operations, residual risk.
...
**Example:** `Refinery Gate Record` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `core checks` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Evidence Inventory

**Fill:** List source files, links, logs, decisions, reviewer records, tests, certificates, hashes, and approvals.

**Example:** In CareQueue Pro, record how `test evidence` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Trace Closure

**Fill:** Show forward and reverse links across intent, requirements, design, code, tests, operations, and risks.

**Example:** In CareQueue Pro, record how `trace evidence` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Decision and Residual Risk

**Fill:** Record verdict, conditions, waivers, residual risk, owner, expiry, and revalidation trigger.

**Example:** In CareQueue Pro, record how `rubric evidence` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Replay and Auditability

**Fill:** Explain how a fresh reviewer can verify, rerun, or resume from this record.

**Example:** In CareQueue Pro, record how `gate verdict` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Refinery Gate Record` focus: core checks | TST-ACPT-019, REV-EVD-007-001 | Example only |
...
1. Artifact captures core checks with owner, evidence, and trace links.
2. Artifact captures test evidence with owner, evidence, and trace links.
3. Artifact captures trace evidence with owner, evidence, and trace links.
4. Artifact captures rubric evidence with owner, evidence, and trace links.
5. Artifact captures gate verdict with owner, evidence, and trace links.
...
1. EVD-007-C1 Quality Gate Critic: validates gate scope, core checks, verdict, and failed-point resolution.
2. EVD-007-C2 Verification Critic: validates tests, evidence, rerunability, and unresolved gaps.
3. EVD-007-C3 Governance Critic: validates trace links, panel records, waivers, and certificate readiness.
...
| Example review panel | `reviews/evd-007-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: core checks, test evidence, trace evidence, rubric evidence, gate verdict.
- Example decision: accept this artifact only after the Quality Lead, Test Lead, Governance Auditor panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Refinery Gate Record` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `core checks`, `test evidence`, and `trace evidence` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `core checks`.
