...
# EVD-006 Residual Risk Acceptance Template

Status: template plus fictional worked example.

Template family: `evidence-certification`.

Use this template when a DFMS factory run needs `Residual Risk Acceptance` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: Risk management.
...
| --- | --- |
| Artifact ID | EVD-006 |
| Artifact name | Residual Risk Acceptance |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Residual Risk Acceptance` evidence;
...
- Related artifacts: Risks, owner, expiration, mitigation.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Human Owner, Security Engineer, Delivery Manager.

## Required Links

Catalog required links: Risks, owner, expiration, mitigation.
...
**Example:** `Residual Risk Acceptance` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `risk owner` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Evidence Inventory

**Fill:** List source files, links, logs, decisions, reviewer records, tests, certificates, hashes, and approvals.

**Example:** In CareQueue Pro, record how `accepted risk` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Trace Closure

**Fill:** Show forward and reverse links across intent, requirements, design, code, tests, operations, and risks.

**Example:** In CareQueue Pro, record how `mitigation` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
**Example:** In CareQueue Pro, record how `monitoring` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Residual Risk Acceptance` focus: risk owner | TST-ACPT-019, REV-EVD-006-001 | Example only |
...
1. Artifact captures risk owner with owner, evidence, and trace links.
2. Artifact captures accepted risk with owner, evidence, and trace links.
3. Artifact captures mitigation with owner, evidence, and trace links.
4. Artifact captures expiry with owner, evidence, and trace links.
5. Artifact captures monitoring with owner, evidence, and trace links.
...
1. EVD-006-C1 Human Risk Owner Critic: validates owner authority, informed acceptance, and business rationale.
2. EVD-006-C2 Security Risk Critic: validates risk severity, mitigation, monitoring, and expiry.
3. EVD-006-C3 Delivery Risk Critic: validates impact, contingency, revalidation trigger, and communication.
...
| Example review panel | `reviews/evd-006-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: risk owner, accepted risk, mitigation, expiry, monitoring.
- Example decision: accept this artifact only after the Human Owner, Security Engineer, Delivery Manager panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Residual Risk Acceptance` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `risk owner`, `accepted risk`, and `mitigation` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `risk owner`.
