...
# EVD-003 Rubric Scorecard Template

Status: template plus fictional worked example.

Template family: `evidence-certification`.

Use this template when a DFMS factory run needs `Rubric Scorecard` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: DFMS quality.
...
| --- | --- |
| Artifact ID | EVD-003 |
| Artifact name | Rubric Scorecard |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Rubric Scorecard` evidence;
...
- Related artifacts: Artifact, reviewers, scores, failures.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Quality Lead, Governance Auditor, Selected Experts.

## Required Links

Catalog required links: Artifact, reviewers, scores, failures.
...
**Example:** `Rubric Scorecard` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `3 reviewers` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Evidence Inventory

**Fill:** List source files, links, logs, decisions, reviewer records, tests, certificates, hashes, and approvals.

**Example:** In CareQueue Pro, record how `15 checks` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Trace Closure

**Fill:** Show forward and reverse links across intent, requirements, design, code, tests, operations, and risks.

**Example:** In CareQueue Pro, record how `scores` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Decision and Residual Risk

**Fill:** Record verdict, conditions, waivers, residual risk, owner, expiry, and revalidation trigger.

**Example:** In CareQueue Pro, record how `failed points` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Replay and Auditability

**Fill:** Explain how a fresh reviewer can verify, rerun, or resume from this record.

**Example:** In CareQueue Pro, record how `fix evidence` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Rubric Scorecard` focus: 3 reviewers | TST-ACPT-019, REV-EVD-003-001 | Example only |
...
1. Artifact captures 3 reviewers with owner, evidence, and trace links.
2. Artifact captures 15 checks with owner, evidence, and trace links.
3. Artifact captures scores with owner, evidence, and trace links.
4. Artifact captures failed points with owner, evidence, and trace links.
5. Artifact captures fix evidence with owner, evidence, and trace links.
...
1. EVD-003-C1 Quality Scoring Critic: validates 3 reviewers, 15 checks each, thresholds, and scoring rationale.
2. EVD-003-C2 Governance Audit Critic: validates evidence links, waivers, failed-point fixes, and certificate compatibility.
3. EVD-003-C3 Selected Expert Consistency Critic: validates that selected experts match artifact risk and panel matrix.
...
| Example review panel | `reviews/evd-003-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: 3 reviewers, 15 checks, scores, failed points, fix evidence.
- Example decision: accept this artifact only after the Quality Lead, Governance Auditor, Selected Experts panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Rubric Scorecard` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `3 reviewers`, `15 checks`, and `scores` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `3 reviewers`.
