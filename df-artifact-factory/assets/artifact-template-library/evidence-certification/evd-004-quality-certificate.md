...
# EVD-004 Quality Certificate Template

Status: template plus fictional worked example.

Template family: `evidence-certification`.

Use this template when a DFMS factory run needs `Quality Certificate` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: DFMS gate.
...
| --- | --- |
| Artifact ID | EVD-004 |
| Artifact name | Quality Certificate |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Quality Certificate` evidence;
...
- Related artifacts: Artifact, evidence, thresholds, residual risk.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Quality Lead, Governance Auditor, Human Owner.

## Required Links

Catalog required links: Artifact, evidence, thresholds, residual risk.
...
**Example:** `Quality Certificate` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `artifact verdict` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Evidence Inventory

**Fill:** List source files, links, logs, decisions, reviewer records, tests, certificates, hashes, and approvals.

**Example:** In CareQueue Pro, record how `thresholds` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Trace Closure

**Fill:** Show forward and reverse links across intent, requirements, design, code, tests, operations, and risks.

**Example:** In CareQueue Pro, record how `evidence bundle` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Decision and Residual Risk

**Fill:** Record verdict, conditions, waivers, residual risk, owner, expiry, and revalidation trigger.

**Example:** In CareQueue Pro, record how `conditions` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Replay and Auditability

**Fill:** Explain how a fresh reviewer can verify, rerun, or resume from this record.

**Example:** In CareQueue Pro, record how `expiry` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Quality Certificate` focus: artifact verdict | TST-ACPT-019, REV-EVD-004-001 | Example only |
...
1. Artifact captures artifact verdict with owner, evidence, and trace links.
2. Artifact captures thresholds with owner, evidence, and trace links.
3. Artifact captures evidence bundle with owner, evidence, and trace links.
4. Artifact captures conditions with owner, evidence, and trace links.
5. Artifact captures expiry with owner, evidence, and trace links.
...
1. EVD-004-C1 Certificate Authority Critic: validates artifact identity, verdict, threshold, and acceptance scope.
2. EVD-004-C2 Governance Evidence Critic: validates referenced panel, refinery gate, trace evidence, and residual risk.
3. EVD-004-C3 Human Acceptance Critic: validates owner approval, conditions, expiry, and revalidation triggers.
...
| Example review panel | `reviews/evd-004-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: artifact verdict, thresholds, evidence bundle, conditions, expiry.
- Example decision: accept this artifact only after the Quality Lead, Governance Auditor, Human Owner panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Quality Certificate` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `artifact verdict`, `thresholds`, and `evidence bundle` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `artifact verdict`.
