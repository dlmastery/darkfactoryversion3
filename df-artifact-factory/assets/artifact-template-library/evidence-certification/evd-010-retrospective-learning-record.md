...
# EVD-010 Retrospective Learning Record Template

Status: template plus fictional worked example.

Template family: `evidence-certification`.

Use this template when a DFMS factory run needs `Retrospective Learning Record` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: Fabro/Gas Town/DFMS continuous improvement.
...
| --- | --- |
| Artifact ID | EVD-010 |
| Artifact name | Retrospective Learning Record |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Retrospective Learning Record` evidence;
...
- Related artifacts: Friction, failures, lessons, template/rubric/skill updates.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Feedback Lead, Governance Auditor, Human Owner.

## Required Links

Catalog required links: Friction, failures, lessons, template/rubric/skill updates.
...
**Example:** `Retrospective Learning Record` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `lesson` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Evidence Inventory

**Fill:** List source files, links, logs, decisions, reviewer records, tests, certificates, hashes, and approvals.

**Example:** In CareQueue Pro, record how `root cause` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Trace Closure

**Fill:** Show forward and reverse links across intent, requirements, design, code, tests, operations, and risks.

**Example:** In CareQueue Pro, record how `process update` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Decision and Residual Risk

**Fill:** Record verdict, conditions, waivers, residual risk, owner, expiry, and revalidation trigger.

**Example:** In CareQueue Pro, record how `regression check` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Replay and Auditability

**Fill:** Explain how a fresh reviewer can verify, rerun, or resume from this record.

**Example:** In CareQueue Pro, record how `future trigger` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Retrospective Learning Record` focus: lesson | TST-ACPT-019, REV-EVD-010-001 | Example only |
...
1. Artifact captures lesson with owner, evidence, and trace links.
2. Artifact captures root cause with owner, evidence, and trace links.
3. Artifact captures process update with owner, evidence, and trace links.
4. Artifact captures regression check with owner, evidence, and trace links.
5. Artifact captures future trigger with owner, evidence, and trace links.
...
1. EVD-010-C1 Feedback Learning Critic: validates lessons, root causes, process updates, and scenario/rubric changes.
2. EVD-010-C2 Governance Improvement Critic: validates owner, approval, rollout, regression impact, and rollback path.
3. EVD-010-C3 Human Adoption Critic: validates whether learning changes future behavior, training, and project-book memory.
...
| Example review panel | `reviews/evd-010-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: lesson, root cause, process update, regression check, future trigger.
- Example decision: accept this artifact only after the Feedback Lead, Governance Auditor, Human Owner panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Retrospective Learning Record` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `lesson`, `root cause`, and `process update` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `lesson`.
