...
# DDD-003 Anti-Corruption Layer Plan Template

Status: template plus fictional worked example.

Template family: `mda-ddd-modeling`.

Use this template when a DFMS factory run needs `Anti-Corruption Layer Plan` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: DDD integration.

Family anchors:

- Domain-Driven Design strategic and tactical modeling
- bounded context and ubiquitous language discipline
- TDD-backed domain rules
- DFMS model traceability
...
| --- | --- |
| Artifact ID | DDD-003 |
| Artifact name | Anti-Corruption Layer Plan |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Anti-Corruption Layer Plan` evidence;
...
- Related artifacts: External systems, adapters, contracts.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: DDD Critic, Security Engineer, System Architect.

## Required Links

Catalog required links: External systems, adapters, contracts.
...
**Example:** `Anti-Corruption Layer Plan` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Fill:** State domain goals, subdomains, stakeholder language, events, policies, and business rules.

**Example:** In CareQueue Pro, record how `external model` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Bounded Context Design

**Fill:** Define context boundaries, ownership, context relationships, translations, and integration style.

**Example:** In CareQueue Pro, record how `adapter` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Tactical Model

**Fill:** Describe aggregates, entities, value objects, repositories, services, invariants, and domain events.

**Example:** In CareQueue Pro, record how `translation rule` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Language and Consistency

**Fill:** Show how ubiquitous language appears in requirements, APIs, code, tests, and support language.

**Example:** In CareQueue Pro, record how `contract test` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Domain Verification

**Fill:** Map invariants and scenarios to tests, examples, edge cases, and operational monitoring.

**Example:** In CareQueue Pro, record how `failure isolation` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Anti-Corruption Layer Plan` focus: external model | TST-ACPT-019, REV-DDD-003-001 | Example only |
...
1. Artifact captures external model with owner, evidence, and trace links.
2. Artifact captures adapter with owner, evidence, and trace links.
3. Artifact captures translation rule with owner, evidence, and trace links.
4. Artifact captures contract test with owner, evidence, and trace links.
5. Artifact captures failure isolation with owner, evidence, and trace links.
...
1. DDD-003-C1 Integration Boundary Critic: validates external system boundaries, translation, adapters, and semantic isolation.
2. DDD-003-C2 Security Critic: validates trust boundary, validation, credential handling, and data leakage controls.
3. DDD-003-C3 Architecture Critic: validates dependency direction, failure handling, retries, and contract testing.
...
| Example review panel | `reviews/ddd-003-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: external model, adapter, translation rule, contract test, failure isolation.
- Example decision: accept this artifact only after the DDD Critic, Security Engineer, System Architect panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Anti-Corruption Layer Plan` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `external model`, `adapter`, and `translation rule` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `external model`.
