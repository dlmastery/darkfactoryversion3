...
# DDD-002 Aggregate and Invariant Catalog Template

Status: template plus fictional worked example.

Template family: `mda-ddd-modeling`.

Use this template when a DFMS factory run needs `Aggregate and Invariant Catalog` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: DDD/TDD.
...
| --- | --- |
| Artifact ID | DDD-002 |
| Artifact name | Aggregate and Invariant Catalog |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Aggregate and Invariant Catalog` evidence;
...
- Related artifacts: Business rules, tests, code.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: DDD Critic, Test Lead, Implementation Lead.

## Required Links

Catalog required links: Business rules, tests, code.
...
**Example:** `Aggregate and Invariant Catalog` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `aggregates` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Bounded Context Design

**Fill:** Define context boundaries, ownership, context relationships, translations, and integration style.

**Example:** In CareQueue Pro, record how `invariants` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Tactical Model

**Fill:** Describe aggregates, entities, value objects, repositories, services, invariants, and domain events.

**Example:** In CareQueue Pro, record how `commands` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Language and Consistency

**Fill:** Show how ubiquitous language appears in requirements, APIs, code, tests, and support language.

**Example:** In CareQueue Pro, record how `events` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Domain Verification

**Fill:** Map invariants and scenarios to tests, examples, edge cases, and operational monitoring.

**Example:** In CareQueue Pro, record how `business-rule tests` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Aggregate and Invariant Catalog` focus: aggregates | TST-ACPT-019, REV-DDD-002-001 | Example only |
...
1. Artifact captures aggregates with owner, evidence, and trace links.
2. Artifact captures invariants with owner, evidence, and trace links.
3. Artifact captures commands with owner, evidence, and trace links.
4. Artifact captures events with owner, evidence, and trace links.
5. Artifact captures business-rule tests with owner, evidence, and trace links.
...
1. DDD-002-C1 Aggregate Design Critic: validates aggregate boundaries, invariants, consistency rules, and transactional scope.
2. DDD-002-C2 Test and TDD Critic: validates business-rule tests, edge cases, fixtures, and regression coverage.
3. DDD-002-C3 Implementation Critic: validates code mapping, persistence constraints, and maintainability.
...
| Example review panel | `reviews/ddd-002-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: aggregates, invariants, commands, events, business-rule tests.
- Example decision: accept this artifact only after the DDD Critic, Test Lead, Implementation Lead panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Aggregate and Invariant Catalog` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `aggregates`, `invariants`, and `commands` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `aggregates`.
