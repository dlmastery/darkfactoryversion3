...
# DDD-001 Bounded Context Map Template

Status: template plus fictional worked example.

Template family: `mda-ddd-modeling`.

Use this template when a DFMS factory run needs `Bounded Context Map` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.
...
| --- | --- |
| Artifact ID | DDD-001 |
| Artifact name | Bounded Context Map |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Bounded Context Map` evidence;
...
- Related artifacts: Glossary, aggregates, services, integrations.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: DDD Critic, System Architect, Domain Expert.

## Required Links

Catalog required links: Glossary, aggregates, services, integrations.
...
**Example:** `Bounded Context Map` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `bounded contexts` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Bounded Context Design

**Fill:** Define context boundaries, ownership, context relationships, translations, and integration style.

**Example:** In CareQueue Pro, record how `context relationships` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Tactical Model

**Fill:** Describe aggregates, entities, value objects, repositories, services, invariants, and domain events.

**Example:** In CareQueue Pro, record how `upstream/downstream` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Language and Consistency

**Fill:** Show how ubiquitous language appears in requirements, APIs, code, tests, and support language.

**Example:** In CareQueue Pro, record how `translation` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Domain Verification

**Fill:** Map invariants and scenarios to tests, examples, edge cases, and operational monitoring.

**Example:** In CareQueue Pro, record how `ownership` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Bounded Context Map` focus: bounded contexts | TST-ACPT-019, REV-DDD-001-001 | Example only |
...
1. Artifact captures bounded contexts with owner, evidence, and trace links.
2. Artifact captures context relationships with owner, evidence, and trace links.
3. Artifact captures upstream/downstream with owner, evidence, and trace links.
4. Artifact captures translation with owner, evidence, and trace links.
5. Artifact captures ownership with owner, evidence, and trace links.
...
1. DDD-001-C1 Context Boundary Critic: validates contexts, ownership, relationships, translations, and integration patterns.
2. DDD-001-C2 System Architecture Critic: validates service/module boundaries, dependencies, and coupling.
3. DDD-001-C3 Domain Language Critic: validates stakeholder terms, context-specific meanings, and glossary trace.
...
| Example review panel | `reviews/ddd-001-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: bounded contexts, context relationships, upstream/downstream, translation, ownership.
- Example decision: accept this artifact only after the DDD Critic, System Architect, Domain Expert panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Bounded Context Map` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `bounded contexts`, `context relationships`, and `upstream/downstream` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `bounded contexts`.
