...
# MDA-004 Model Transformation Record Template

Status: template plus fictional worked example.

Template family: `mda-ddd-modeling`.

Use this template when a DFMS factory run needs `Model Transformation Record` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.
...
| --- | --- |
| Artifact ID | MDA-004 |
| Artifact name | Model Transformation Record |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Model Transformation Record` evidence;
...
- Related artifacts: CIM->PIM->PSM mappings and exceptions.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: MDA Architect, Governance Auditor, Test Lead.

## Required Links

Catalog required links: CIM->PIM->PSM mappings and exceptions.
...
**Example:** `Model Transformation Record` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `mapping table` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Model Elements

**Fill:** List entities, value objects, services, interactions, state, events, constraints, and diagrams.

**Example:** In CareQueue Pro, record how `transformation rule` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Transformation Links

**Fill:** Trace CIM, PIM, PSM, exceptions, losses, platform decisions, and transformation rationale.

**Example:** In CareQueue Pro, record how `exception` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Model Consistency

**Fill:** Check terminology, invariants, interfaces, quality attributes, and stakeholder validation.

**Example:** In CareQueue Pro, record how `lost information` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Model Verification

**Fill:** Map model elements to requirements, tests, code, data, and review evidence.

**Example:** In CareQueue Pro, record how `verification evidence` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Model Transformation Record` focus: mapping table | TST-ACPT-019, REV-MDA-004-001 | Example only |
...
1. Artifact captures mapping table with owner, evidence, and trace links.
2. Artifact captures transformation rule with owner, evidence, and trace links.
3. Artifact captures exception with owner, evidence, and trace links.
4. Artifact captures lost information with owner, evidence, and trace links.
5. Artifact captures verification evidence with owner, evidence, and trace links.
...
1. MDA-004-C1 Transformation Integrity Critic: validates CIM-to-PIM-to-PSM mappings, losses, exceptions, and rationale.
2. MDA-004-C2 Governance Trace Critic: validates requirement/model links, approvals, waivers, and auditability.
3. MDA-004-C3 Verification Critic: validates transformation tests, model reviews, and implementation coverage.
...
| Example review panel | `reviews/mda-004-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: mapping table, transformation rule, exception, lost information, verification evidence.
- Example decision: accept this artifact only after the MDA Architect, Governance Auditor, Test Lead panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Model Transformation Record` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `mapping table`, `transformation rule`, and `exception` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `mapping table`.
