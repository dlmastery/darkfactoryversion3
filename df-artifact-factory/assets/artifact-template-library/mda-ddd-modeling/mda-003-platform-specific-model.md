...
# MDA-003 Platform-Specific Model Template

Status: template plus fictional worked example.

Template family: `mda-ddd-modeling`.

Use this template when a DFMS factory run needs `Platform-Specific Model` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.
...
| --- | --- |
| Artifact ID | MDA-003 |
| Artifact name | Platform-Specific Model |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Platform-Specific Model` evidence;
...
- Related artifacts: PIM mappings, stack choices, code modules.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: MDA Architect, Implementation Lead, SRE Lead.

## Required Links

Catalog required links: PIM mappings, stack choices, code modules.
...
**Example:** `Platform-Specific Model` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `runtime stack` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Model Elements

**Fill:** List entities, value objects, services, interactions, state, events, constraints, and diagrams.

**Example:** In CareQueue Pro, record how `module mapping` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Transformation Links

**Fill:** Trace CIM, PIM, PSM, exceptions, losses, platform decisions, and transformation rationale.

**Example:** In CareQueue Pro, record how `deployment units` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Model Consistency

**Fill:** Check terminology, invariants, interfaces, quality attributes, and stakeholder validation.

**Example:** In CareQueue Pro, record how `config` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Model Verification

**Fill:** Map model elements to requirements, tests, code, data, and review evidence.

**Example:** In CareQueue Pro, record how `platform constraints` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Platform-Specific Model` focus: runtime stack | TST-ACPT-019, REV-MDA-003-001 | Example only |
...
1. Artifact captures runtime stack with owner, evidence, and trace links.
2. Artifact captures module mapping with owner, evidence, and trace links.
3. Artifact captures deployment units with owner, evidence, and trace links.
4. Artifact captures config with owner, evidence, and trace links.
5. Artifact captures platform constraints with owner, evidence, and trace links.
...
1. MDA-003-C1 Platform Mapping Critic: validates stack choices, code-module mapping, runtime constraints, and deployment fit.
2. MDA-003-C2 Implementation Critic: validates feasibility, repo layout, dependencies, and developer maintainability.
3. MDA-003-C3 SRE Runtime Critic: validates config, scaling, observability, rollout, and operational risk.
...
| Example review panel | `reviews/mda-003-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: runtime stack, module mapping, deployment units, config, platform constraints.
- Example decision: accept this artifact only after the MDA Architect, Implementation Lead, SRE Lead panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Platform-Specific Model` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `runtime stack`, `module mapping`, and `deployment units` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `runtime stack`.
