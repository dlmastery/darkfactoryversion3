...
# ARC-005 Data Model and Migration Plan Template

Status: template plus fictional worked example.

Template family: `architecture-design`.

Use this template when a DFMS factory run needs `Data Model and Migration Plan` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: DDD/data architecture.
...
| --- | --- |
| Artifact ID | ARC-005 |
| Artifact name | Data Model and Migration Plan |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Data Model and Migration Plan` evidence;
...
- Related artifacts: Entities, stores, privacy, migrations.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: Data Architect, Security Engineer, SRE Lead.

## Required Links

Catalog required links: Entities, stores, privacy, migrations.
...
**Example:** `Data Model and Migration Plan` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `entities` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Design Structure

**Fill:** Describe boundaries, responsibilities, data/control flows, interfaces, and dependency direction.

**Example:** In CareQueue Pro, record how `relationships` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Alternatives and Tradeoffs

**Fill:** Compare viable alternatives, rejected options, decision rationale, reversibility, and consequences.

**Example:** In CareQueue Pro, record how `migration steps` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Failure, Security, and Operations

**Fill:** Capture threat boundaries, failure modes, observability, degradation, and rollback considerations.

**Example:** In CareQueue Pro, record how `privacy handling` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
**Example:** In CareQueue Pro, record how `reconciliation checks` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Data Model and Migration Plan` focus: entities | TST-ACPT-019, REV-ARC-005-001 | Example only |
...
1. Artifact captures entities with owner, evidence, and trace links.
2. Artifact captures relationships with owner, evidence, and trace links.
3. Artifact captures migration steps with owner, evidence, and trace links.
4. Artifact captures privacy handling with owner, evidence, and trace links.
5. Artifact captures reconciliation checks with owner, evidence, and trace links.
...
1. ARC-005-C1 Data Architecture Critic: validates entities, relationships, ownership, lifecycle, and data quality.
2. ARC-005-C2 Security and Privacy Critic: validates classification, retention, access, masking, and compliance.
3. ARC-005-C3 Migration Reliability Critic: validates backfill, validation queries, rollback, performance, and operational window.
...
| Example review panel | `reviews/arc-005-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: entities, relationships, migration steps, privacy handling, reconciliation checks.
- Example decision: accept this artifact only after the Data Architect, Security Engineer, SRE Lead panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Data Model and Migration Plan` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `entities`, `relationships`, and `migration steps` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `entities`.
