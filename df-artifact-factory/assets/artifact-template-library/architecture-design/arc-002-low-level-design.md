...
# ARC-002 Low-Level Design Template

Status: template plus fictional worked example.

Template family: `architecture-design`.

Use this template when a DFMS factory run needs `Low-Level Design` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: ISO implementation planning.
...
| --- | --- |
| Artifact ID | ARC-002 |
| Artifact name | Low-Level Design |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Low-Level Design` evidence;
...
- Related artifacts: Components, APIs, data model, tests.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: System Architect, Implementation Lead, Test Lead.

## Required Links

Catalog required links: Components, APIs, data model, tests.
...
**Example:** `Low-Level Design` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `component contracts` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Design Structure

**Fill:** Describe boundaries, responsibilities, data/control flows, interfaces, and dependency direction.

**Example:** In CareQueue Pro, record how `algorithms` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Alternatives and Tradeoffs

**Fill:** Compare viable alternatives, rejected options, decision rationale, reversibility, and consequences.

**Example:** In CareQueue Pro, record how `data structures` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Failure, Security, and Operations

**Fill:** Capture threat boundaries, failure modes, observability, degradation, and rollback considerations.

**Example:** In CareQueue Pro, record how `API details` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
**Example:** In CareQueue Pro, record how `test seams` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Low-Level Design` focus: component contracts | TST-ACPT-019, REV-ARC-002-001 | Example only |
...
1. Artifact captures component contracts with owner, evidence, and trace links.
2. Artifact captures algorithms with owner, evidence, and trace links.
3. Artifact captures data structures with owner, evidence, and trace links.
4. Artifact captures API details with owner, evidence, and trace links.
5. Artifact captures test seams with owner, evidence, and trace links.
...
1. ARC-002-C1 Component Design Critic: validates component contracts, algorithms, APIs, data handling, and cohesion.
2. ARC-002-C2 Implementation Feasibility Critic: validates repo fit, complexity, dependencies, migration, and conventions.
3. ARC-002-C3 Test Design Critic: validates unit/integration coverage, edge cases, mocks, fixtures, and regression hooks.
...
| Example review panel | `reviews/arc-002-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: component contracts, algorithms, data structures, API details, test seams.
- Example decision: accept this artifact only after the System Architect, Implementation Lead, Test Lead panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Low-Level Design` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `component contracts`, `algorithms`, and `data structures` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `component contracts`.
