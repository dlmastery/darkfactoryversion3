...
# ARC-003 Architecture Decision Records Template

Status: template plus fictional worked example.

Template family: `architecture-design`.

Use this template when a DFMS factory run needs `Architecture Decision Records` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: ADR practice, ISO rationale records.
...
| --- | --- |
| Artifact ID | ARC-003 |
| Artifact name | Architecture Decision Records |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Architecture Decision Records` evidence;
...
- Related artifacts: Alternatives, chosen design, consequences.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: System Architect, DDD Critic, Governance Auditor.

## Required Links

Catalog required links: Alternatives, chosen design, consequences.
...
**Example:** `Architecture Decision Records` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `decision context` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Design Structure

**Fill:** Describe boundaries, responsibilities, data/control flows, interfaces, and dependency direction.

**Example:** In CareQueue Pro, record how `alternatives` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Alternatives and Tradeoffs

**Fill:** Compare viable alternatives, rejected options, decision rationale, reversibility, and consequences.

**Example:** In CareQueue Pro, record how `chosen option` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Failure, Security, and Operations

**Fill:** Capture threat boundaries, failure modes, observability, degradation, and rollback considerations.

**Example:** In CareQueue Pro, record how `consequences` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
**Example:** In CareQueue Pro, record how `reversal conditions` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Architecture Decision Records` focus: decision context | TST-ACPT-019, REV-ARC-003-001 | Example only |
...
1. Artifact captures decision context with owner, evidence, and trace links.
2. Artifact captures alternatives with owner, evidence, and trace links.
3. Artifact captures chosen option with owner, evidence, and trace links.
4. Artifact captures consequences with owner, evidence, and trace links.
5. Artifact captures reversal conditions with owner, evidence, and trace links.
...
1. ARC-003-C1 Decision Rationale Critic: validates context, options, chosen decision, consequences, and reversibility.
2. ARC-003-C2 Domain and DDD Critic: validates domain alignment, context boundaries, and invariant impact.
3. ARC-003-C3 Governance Audit Critic: validates approval, trace, date/version, residual risk, and rejected alternatives.
...
| Example review panel | `reviews/arc-003-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: decision context, alternatives, chosen option, consequences, reversal conditions.
- Example decision: accept this artifact only after the System Architect, DDD Critic, Governance Auditor panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Architecture Decision Records` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `decision context`, `alternatives`, and `chosen option` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `decision context`.
