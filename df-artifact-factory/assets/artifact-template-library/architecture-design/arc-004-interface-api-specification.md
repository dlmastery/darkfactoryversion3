...
# ARC-004 Interface/API Specification Template

Status: template plus fictional worked example.

Template family: `architecture-design`.

Use this template when a DFMS factory run needs `Interface/API Specification` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: ISO, OpenAPI/gRPC/etc. as applicable.
...
| --- | --- |
| Artifact ID | ARC-004 |
| Artifact name | Interface/API Specification |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `Interface/API Specification` evidence;
...
- Related artifacts: Requirements, consumers, tests.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: System Architect, Test Lead, Security Engineer.

## Required Links

Catalog required links: Requirements, consumers, tests.
...
**Example:** `Interface/API Specification` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `contract schema` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Design Structure

**Fill:** Describe boundaries, responsibilities, data/control flows, interfaces, and dependency direction.

**Example:** In CareQueue Pro, record how `error model` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Alternatives and Tradeoffs

**Fill:** Compare viable alternatives, rejected options, decision rationale, reversibility, and consequences.

**Example:** In CareQueue Pro, record how `auth/authz` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Failure, Security, and Operations

**Fill:** Capture threat boundaries, failure modes, observability, degradation, and rollback considerations.

**Example:** In CareQueue Pro, record how `versioning` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
**Example:** In CareQueue Pro, record how `consumer tests` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `Interface/API Specification` focus: contract schema | TST-ACPT-019, REV-ARC-004-001 | Example only |
...
1. Artifact captures contract schema with owner, evidence, and trace links.
2. Artifact captures error model with owner, evidence, and trace links.
3. Artifact captures auth/authz with owner, evidence, and trace links.
4. Artifact captures versioning with owner, evidence, and trace links.
5. Artifact captures consumer tests with owner, evidence, and trace links.
...
1. ARC-004-C1 API Contract Critic: validates endpoints/messages, schemas, errors, versioning, and consumer fit.
2. ARC-004-C2 Security Contract Critic: validates auth, authorization, input validation, rate limits, and sensitive data.
3. ARC-004-C3 Contract Test Critic: validates examples, compatibility tests, negative cases, and consumer/provider verification.
...
| Example review panel | `reviews/arc-004-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: contract schema, error model, auth/authz, versioning, consumer tests.
- Example decision: accept this artifact only after the System Architect, Test Lead, Security Engineer panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `Interface/API Specification` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `contract schema`, `error model`, and `auth/authz` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `contract schema`.
