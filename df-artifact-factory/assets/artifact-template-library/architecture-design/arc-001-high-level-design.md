...
# ARC-001 High-Level Design Template

Status: template plus fictional worked example.

Template family: `architecture-design`.

Use this template when a DFMS factory run needs `High-Level Design` as a governed SDLC artifact. The template is intentionally standards-based, trace-first, and reviewable by an artifact-specific critic panel.

## Standards and Method Anchors

Primary basis from catalog: RUP, ISO design, architecture practice.
...
| --- | --- |
| Artifact ID | ARC-001 |
| Artifact name | High-Level Design |
...
- the project risk, contract, lifecycle stage, or handoff requires explicit `High-Level Design` evidence;
...
- Related artifacts: Requirements, NFRs, ADRs, risks.
- Current risks, decisions, open questions, and residual-risk records.
- Required reviewers: System Architect, SRE Lead, Security Engineer.

## Required Links

Catalog required links: Requirements, NFRs, ADRs, risks.
...
**Example:** `High-Level Design` for CareQueue Pro covers referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics and is owned by the clinic operations product owner with DFMS delivery accountability.
...
**Example:** In CareQueue Pro, record how `system boundaries` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Design Structure

**Fill:** Describe boundaries, responsibilities, data/control flows, interfaces, and dependency direction.

**Example:** In CareQueue Pro, record how `logical view` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Alternatives and Tradeoffs

**Fill:** Compare viable alternatives, rejected options, decision rationale, reversibility, and consequences.

**Example:** In CareQueue Pro, record how `deployment view` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.

### Failure, Security, and Operations

**Fill:** Capture threat boundaries, failure modes, observability, degradation, and rollback considerations.

**Example:** In CareQueue Pro, record how `failure modes` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
**Example:** In CareQueue Pro, record how `architecture alternatives` affects referral intake, nurse review queue, patient reminder workflow, and operations dashboard for three clinics.
...
| INT-ANS-014 | REQ-032 | `High-Level Design` focus: system boundaries | TST-ACPT-019, REV-ARC-001-001 | Example only |
...
1. Artifact captures system boundaries with owner, evidence, and trace links.
2. Artifact captures logical view with owner, evidence, and trace links.
3. Artifact captures deployment view with owner, evidence, and trace links.
4. Artifact captures failure modes with owner, evidence, and trace links.
5. Artifact captures architecture alternatives with owner, evidence, and trace links.
...
1. ARC-001-C1 System Architecture Critic: validates boundaries, responsibilities, flows, tradeoffs, and alternatives.
2. ARC-001-C2 Security and Trust Critic: validates trust boundaries, data protection, abuse cases, and risk treatment.
3. ARC-001-C3 SRE Operability Critic: validates reliability, observability, deployment, failure modes, and maintainability.
...
| Example review panel | `reviews/arc-001-panel.json` | DFMS quality refinery | artifact-specific critics | example only |
...
- Artifact-specific focus: system boundaries, logical view, deployment view, failure modes, architecture alternatives.
- Example decision: accept this artifact only after the System Architect, SRE Lead, Security Engineer panel resolves failed points and links evidence to the refinery gate.

### Example Artifact Snippet

> For CareQueue Pro, the `High-Level Design` records that the pilot must support referral intake and nurse triage for three clinics while deferring EHR writeback. The artifact links INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004 and remains conditional until the artifact-specific critic panel confirms `system boundaries`, `logical view`, and `deployment view` with evidence.

### Example Review Outcome

- Content authority critic: conditional pass pending one fix to `system boundaries`.
